...
.cat-chip.active{background:linear-gradient(45deg,#ef4444,#dc2626);border-color:#ef4444;color:#fff;box-shadow:0 0 12px rgba(239,68,68,0.4)}
...
.fav-btn{position:absolute;top:10px;right:10px;background:rgba(0,0,0,0.6);border:none;width:30px;height:30px;border-radius:50%;color:#fff;font-size:14px;cursor:pointer;display:flex;align-items:center;justify-content:center;z-index:10}
...
.floating-btn{position:fixed;right:15px;width:50px;height:50px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:20px;z-index:500;cursor:pointer;box-shadow:0 4px 15px rgba(0,0,0,0.5);border:2px solid #fff;text-decoration:none;color:#fff}
...
.dl-file-btn{display:flex;align-items:center;justify-content:space-between;width:100%;padding:15px;background:#0f172a;border:1px solid #334155;color:#fff;font-weight:700;border-radius:10px;margin-bottom:10px;cursor:pointer}
...
.age-yes{background:#ef4444;color:#fff}
.age-no{background:#334155;color:#fff}
...
.ad-box-orange{background:#ea580c;color:#fff;padding:12px;border-radius:8px;margin-bottom:10px;font-weight:600}
...
.btn-ad-open{background:#ea580c;color:#fff}
.btn-ad-unlock{background:#10b981;color:#fff}
.btn-ad-tryagain{background:#ef4444;color:#fff}
...
.profile-action-btn{display:block;width:100%;padding:14px;border-radius:12px;font-weight:700;text-align:center;margin-bottom:10px;border:none;color:#fff;text-decoration:none;cursor:pointer;font-size:15px;transition:0.2s}
...
.join-channel-btn{display:block;width:100%;padding:15px;border-radius:12px;background:#24A1DE;color:#fff;font-weight:700;text-decoration:none;font-size:16px;text-align:center;margin-top:15px;margin-bottom:10px;box-shadow:0 4px 10px rgba(36,161,222,0.3)}
</style>
</head>
<body>
...
<p style="color:#94a3b8;margin-bottom:30px">কী দেখবেন ঠিক করতে পারছেন না?</p>
<button onclick="loadSurprise()" style="padding:15px 40px;background:linear-gradient(45deg,#ff416c,#ff4b2b);color:#fff;border:none;border-radius:30px;font-size:18px;font-weight:800;cursor:pointer">🎲 Surprise Me!</button>
</div>
</div>
...
var DIRECT_LINKS = __DL_JSON__;
var ADULT_DIRECT_LINKS = __ADL_JSON__;
var INIT_DATA = tg.initData || "";
var uid = tg.initDataUnsafe && tg.initDataUnsafe.user ? tg.initDataUnsafe.user.id : 0;
var isUserVip = false; var activeCat = "Home"; var userFavs = []; var active18Btn = null; var activeFileId = null; var activeIsAdult = false; var adStartTime = 0; var currentViewMovies = [];
...
async function fetchUserInfo(){ try{ var r=await fetch('/api/user/'+uid); var d=await r.json(); isUserVip=d.vip; }catch(e){} }
...
function toggleOledMode(){ document.body.classList.toggle('oled-mode'); var sEl=document.getElementById('darkModeStatus'); if(document.body.classList.contains('oled-mode')){ sEl.innerText='ON'; localStorage.setItem('oledMode','true'); }else{ sEl.innerText='OFF'; localStorage.setItem('oledMode','false'); } }
...
async function loadHomeMovies(){
    var list=document.getElementById('movieListHome'); list.innerHTML='<div class="skeleton"></div>';
    try{ var r=await fetch('/api/list?cat='+activeCat+'&uid='+uid); var d=await r.json(); currentViewMovies=d.movies||[];
    if(currentViewMovies.length>0){ list.innerHTML=currentViewMovies.map(function(m,i){ return createMovieCard(m,i); }).join(''); }else{ list.innerHTML='<p style="text-align:center;color:#64748b;padding:30px">কোনো মুভি পাওয়া যায়নি!</p>'; } }catch(e){ list.innerHTML='<p style="text-align:center;color:#ef4444;padding:30px">Error!</p>'; }
}

async function searchMovies(){
    var q=document.getElementById('searchInputMain').value.trim(); var list=document.getElementById('movieListSearch');
    if(!q){ list.innerHTML=''; return; }
    try{ var r=await fetch('/api/list?q='+encodeURIComponent(q)+'&uid='+uid); var d=await r.json(); currentViewMovies=d.movies||[];
    if(currentViewMovies.length>0){ list.innerHTML=currentViewMovies.map(function(m,i){ return createMovieCard(m,i); }).join(''); }else{ list.innerHTML='<p style="text-align:center;color:#64748b">খুঁজে পাওয়া যায়নি!</p>'; } }catch(e){}
}

function createMovieCard(m, index){
    var isFav = userFavs.indexOf(m._id) !== -1;
    var isAdult = m.categories && m.categories.indexOf("Adult Content") !== -1;
    var isVerified = localStorage.getItem('isAdult') === 'true';
    var catsHtml = (m.categories||[]).map(function(c){ return '<span class="movie-cat-tag">'+c+'</span>'; }).join('');
    var imgSrc = (isAdult && !isVerified) ? 'https://via.placeholder.com/110x160/1e293b/ef4444?text=18%2B' : '/api/image/'+m.photo_id;
    var lockOverlay = (isAdult && !isVerified) ? '<div class="adult-lock-overlay"><i class="fa-solid fa-lock"></i></div>' : '';
    var clickAction = (isAdult && !isVerified) ? 'onclick="verify18(null)"' : 'onclick="openDetail('+index+')"';
    var safeId = m._id.replace(/'/g, "\\'");
    return '<div class="movie-card" '+clickAction+'><div style="position:relative;flex-shrink:0"><img src="'+imgSrc+'" style="width:110px;height:160px;object-fit:cover">'+lockOverlay+'</div><div class="movie-info"><div class="movie-title">'+m._id+'</div><div class="movie-meta"><span>'+(m.year||'N/A')+'</span><span>'+(m.files?m.files.length:0)+' Files</span></div><div class="movie-cats">'+catsHtml+'</div></div><button class="fav-btn '+(isFav?'active':'')+'" onclick="event.stopPropagation();toggleFav(\''+safeId+'\',this)"><i class="fa-solid fa-heart"></i></button></div>';
}

function openDetail(index){
    var m=currentViewMovies[index]; if(!m) return;
    ...
    var isAdult=m.is_adult||false;
    var btnsHtml=m.files.map(function(f){ var isFree=f.is_unlocked||isUserVip; return '<button class="dl-file-btn '+(isFree?'unlocked':'')+'" onclick="handleFileClick(\''+f.id+'\','+(isFree?'true':'false')+','+(isAdult?'true':'false')+')"><span><i class="fa-solid fa-'+(isFree?'lock-open':'lock')+'"></i> Download '+f.quality+'</span></button>'; }).join('');
...
function checkAdWatched(){
    ...
    var elapsed=Date.now()-adStartTime;
    if(elapsed>=10000){ closeModal('adModal'); sendFileRequest(activeFileId); }
    else{ var remaining=Math.ceil((10000-elapsed)/1000); tg.showAlert('⚠️ আপনাকে আর '+remaining+' সেকেন্ড অপেক্ষা করতে হবে!'); document.getElementById('adVerifyBtn').style.display='none'; document.getElementById('adTryAgainBtn').style.display='block'; document.getElementById('adTryAgainBtn').innerText='TRY AGAIN'; }
}

async function sendFileRequest(fileId){ try{ var r=await fetch('/api/send',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({userId:uid,movieId:fileId,initData:INIT_DATA})}); var d=await r.json(); if(d.ok){ closeModal('detailModal'); document.getElementById('successModal').style.display='flex'; fetchUserInfo(); }else{ tg.showAlert('⚠️ Failed!'); } }catch(e){} }
...
async function loadSurprise(){ try{ var r=await fetch('/api/random'); var d=await r.json(); if(d.movie){ currentViewMovies=[d.movie]; openDetail(0); }else{ tg.showAlert('⚠️ ডাটাবেসে কোনো মুভি নেই!'); } }catch(e){} }
