...
# ==========================================
# 7. Admin Settings & Profile Commands
# ==========================================
...
async def protect_cmd(m: types.Message):
    ...
    try:
        ...
        if state == "on": await db.settings.update_one({"id": "protect_content"}, {"$set": {"status": True}}, upsert=True); await m.answer("✅ প্রোটেকশন চালু।")
        elif state == "off": await db.settings.update_one({"id": "protect_content"}, {"$set": {"status": False}}, upsert=True); await m.answer("✅ প্রোটেকশন বন্ধ।")
...
async def add_vip_cmd(m: types.Message):
    ...
    try:
        args = m.text.split()
        target_uid = int(args[1]); days = int(args[2]) if len(args) > 2 else 30 
...
async def remove_vip_cmd(m: types.Message):
    if m.from_user.id not in admin_cache: return
    try:
        target_uid = int(m.text.split()[1]); now = datetime.datetime.utcnow()
...
async def receive_movie_photo(m: types.Message, state: FSMContext):
    ...
    await m.answer("✅ পোস্টার পেয়েছি! এবার <b>মুভির নাম</b> লিখুন।", parse_mode="HTML")
...
async def receive_movie_title(m: types.Message, state: FSMContext):
    ...
    await m.answer("✅ নাম সেভ হয়েছে! এবার <b>কোয়ালিটি/এপিসোড</b> দিন।", parse_mode="HTML")
...
async def upc_release_step(m: types.Message, state: FSMContext):
    ...
    kb = types.ReplyKeyboardMarkup(keyboard=[[types.KeyboardButton(text="Bangla"), types.KeyboardButton(text="Hindi")], [types.KeyboardButton(text="English"), types.KeyboardButton(text="Korean")]], resize_keyboard=True)
...
async def upc_lang_step(m: types.Message, state: FSMContext):
    data = await state.get_data(); await state.clear(); rm = types.ReplyKeyboardRemove()
    await db.upcoming.insert_one({"photo_id": data["photo_id"], "title": data["title"], "release_date": data["release_date"], "language": m.text.strip(), "added_at": datetime.datetime.utcnow()})
...
async def del_upc_cmd(m: types.Message):
    if m.from_user.id not in admin_cache: return
    await db.upcoming.delete_many({}); await m.answer("🗑 সব আপকামিং মুভি ডিলিট করা হয়েছে!")
...
async def broadcast_prep(m: types.Message, state: FSMContext):
    if m.from_user.id not in admin_cache: return
    await state.set_state(AdminStates.waiting_for_bcast); await m.answer("📢 যে মেসেজটি ব্রডকাস্ট করতে চান সেটি পাঠান।")

@dp.message(AdminStates.waiting_for_bcast)
async def execute_broadcast(m: types.Message, state: FSMContext):
    await state.clear(); await m.answer("⏳ ব্রডকাস্ট শুরু হয়েছে...")
    success = 0
...
async def process_reply_cb(c: types.CallbackQuery, state: FSMContext):
    if c.from_user.id not in admin_cache: return
    user_id = int(c.data.split("_")[1]); await state.set_state(AdminStates.waiting_for_reply); await state.update_data(target_uid=user_id)
    await c.message.reply("✍️ <b>ইউজারকে কী রিপ্লাই দিতে চান তা লিখুন:</b>", parse_mode="HTML"); await c.answer()
...
async def web_admin_panel(auth: bool = Depends(verify_admin)):
    return HTMLResponse("<h1>Admin Panel Running</h1><script>window.location.href='/admin/data'</script>")

@app.get("/api/admin/data")
async def get_admin_data(auth: bool = Depends(verify_admin)):
    uc = await db.users.count_documents({}); mc = await db.movies.count_documents({})
    return {"total_users": uc, "total_movies": mc}
...
async def web_ui():
    ...
    dl_json = json.dumps(dl_cfg.get('links', []) if dl_cfg else [])
    tg_url = tg_cfg['url'] if tg_cfg else "https://t.me/MovieeBD"
    
    s_data = {"channel": social_cfg.get("channel", "#") if social_cfg else "#", "fb": social_cfg.get("facebook", "#") if social_cfg else "#", "yt": social_cfg.get("youtube", "#") if social_cfg else "#", "web": social_cfg.get("website", "#") if social_cfg else "#", "about": social_cfg.get("about", "Movie Box BD Official App") if social_cfg else "Movie Box BD Official App"}
    social_json = json.dumps(s_data)
    ...
    <head>
        ...
        <style>
            * { margin: 0; padding: 0; box-sizing: border-box; }
            body { background: #0f172a; font-family: 'Segoe UI', sans-serif; color: #fff; padding-bottom: 70px; } 
            ...
            .welcome-text { font-size: 28px; font-weight: 900; background: linear-gradient(45deg, #00f260, #0575e6); -webkit-background-clip: text; -webkit-text-fill-color: transparent; animation: pulse 1.5s infinite; }
            @keyframes pulse { 0% { transform: scale(1); } 50% { transform: scale(1.05); } 100% { transform: scale(1); } }

            /* Header */
            header { display: flex; justify-content: center; align-items: center; padding: 15px; border-bottom: 1px solid rgba(255,255,255,0.05); position: sticky; top: 0; background: rgba(15, 23, 42, 0.85); backdrop-filter: blur(20px); z-index: 1000; }
            .logo { font-size: 24px; font-weight: 900; color: #fff; text-shadow: 0 0 10px rgba(0, 245, 96, 0.5); cursor: pointer; letter-spacing: 2px; }

            /* App Pages */
            .app-page { display: none; } .app-page.active { display: block; animation: fadeIn 0.3s ease; }
            @keyframes fadeIn { from { opacity: 0; } to { opacity: 1; } }

            /* Search & Categories */
            .search-box { padding: 15px; }
            .search-input { width: 100%; padding: 14px; border-radius: 15px; border: 1px solid rgba(255,255,255,0.1); outline: none; background: rgba(255,255,255,0.05); backdrop-filter: blur(10px); color: #fff; font-size: 16px; transition: 0.3s; }
            ...
            .section-title { padding: 10px 15px; font-size: 18px; font-weight: 800; color: #e2e8f0; }
            ...
            .movie-meta { font-size: 12px; color: #94a3b8; display: flex; gap: 10px; }
            ...
            .modal-content { background: rgba(30, 41, 59, 0.95); backdrop-filter: blur(20px); width: 92%; max-width: 400px; padding: 25px; border-radius: 20px; text-align: center; border: 1px solid rgba(255,255,255,0.1); position: relative; }
            .close-icon { position: absolute; top: 12px; right: 15px; width: 32px; height: 32px; border-radius: 50%; background: rgba(255,255,255,0.1); color: #fff; display: flex; align-items: center; justify-content: center; cursor: pointer; z-index: 100; border: none; }
            ...
            .social-btn:active { background: #00f260; color: #000; }
    ...
    <body>
        <div id="welcomeScreen"><div style="font-size: 60px; margin-bottom: 20px;">🎬</div><div class="welcome-text">মুভি বক্স জগতে স্বাগতম</div></div>
        <header><div class="logo" onclick="switchPage('home')">Movie Box</div></header>

        <div id="page-home" class="app-page active">
            <div class="search-box"><input type="text" id="searchInput" class="search-input" placeholder="🔍 মুভি বা সিরিজ খুঁজুন..."></div>
            <div class="cat-container" id="catContainer">
                <div class="cat-btn active" onclick="filterCat('All')">All</div><div class="cat-btn" onclick="filterCat('Bangla')">Bangla</div><div class="cat-btn" onclick="filterCat('Bengali Dubbed')">Dubbed</div><div class="cat-btn" onclick="filterCat('Hindi')">Hindi</div><div class="cat-btn" onclick="filterCat('Hindi Dubbed')">Hindi Dubbed</div><div class="cat-btn" onclick="filterCat('English')">English</div><div class="cat-btn" onclick="filterCat('Web Series')">Web Series</div><div class="cat-btn" onclick="filterCat('Korean')">Korean</div><div class="cat-btn" onclick="filterCat('Anime')">Anime</div><div class="cat-btn" onclick="verifyAge()">18+</div>
            </div>
        ...
        <script>
            ...
            let currentCat = "All"; let activeMovieData = null; let isFav = false; let dlUnlocked = false; let timerInterval = null; let isUserVip = false;

            setTimeout(() => { document.getElementById('welcomeScreen').classList.add('hide'); }, 2500);

            if(tg.initDataUnsafe?.user) { document.getElementById('profName').innerText = tg.initDataUnsafe.user.first_name; if(tg.initDataUnsafe.user.photo_url) document.getElementById('profPic').src = tg.initDataUnsafe.user.photo_url; }
            loadSocialLinks(); fetchUserInfo();
            ...
            function verifyAge() { openModal('ageModal'); }
            function confirmAge() { closeModal('ageModal'); currentCat = '18+'; document.querySelectorAll('.cat-btn').forEach(b => b.classList.remove('active')); event.target.closest('.cat-btn')?.classList.add('active'); loadMovies(); }
            ...
            async function loadMovies() {
                ...
                try {
                    const r = await fetch(`/api/list?page=1&cat=${currentCat}&q=${document.getElementById('searchInput').value}&uid=${uid}`);
                        ...
                        grid.innerHTML = data.movies.map(m => `
                            <div class="movie-row" onclick="openDetail('${m._id.replace(/'/g, "\\'")}')">
                                ...
                                <div class="movie-info">
                                    ...
                                    <div class="movie-meta"><span><i class="fa-solid fa-eye"></i> ${m.clicks}</span><span><i class="fa-solid fa-list"></i> ${m.files.length} Files</span></div>
                                    <div style="margin-top:5px;"><span class="badge-cat">${m.category || 'General'}</span></div>
                                </div>
                            </div>
                        `).join('');
                    } else { grid.innerHTML = '<p style="text-align:center; color:#94a3b8; padding:20px;">কোনো মুভি পাওয়া যায়নি!</p>'; }
                } catch(e) {}
            }

            let searchTimeout = null;
            document.getElementById('searchInput').addEventListener('input', (e) => { clearTimeout(searchTimeout); searchTimeout = setTimeout(() => loadMovies(), 500); });

            async function searchFull() {
                const q = document.getElementById('searchInputFull').value; const grid = document.getElementById('searchResults');
                if(!q) return grid.innerHTML = '';
                try {
                    const r = await fetch(`/api/list?page=1&cat=All&q=${q}&uid=${uid}`); const data = await r.json();
                    if(data.movies && data.movies.length > 0) { grid.innerHTML = data.movies.map(m => `<div class="movie-row" onclick="openDetail('${m._id.replace(/'/g, "\\'")}')"><img class="movie-poster" src="/api/image/${m.photo_id}"><div class="movie-info"><div class="movie-title-text">${m._id}</div><div class="movie-meta"><span class="badge-cat">${m.category || 'General'}</span></div></div></div>`).join(''); }
                    else { grid.innerHTML = '<p style="text-align:center; color:#94a3b8;">পাওয়া যায়নি!</p>'; }
                } catch(e) {}
            }

            async function openDetail(title) {
                const r = await fetch(`/api/list?page=1&cat=All&q=${title}&uid=${uid}`); const data = await r.json();
                if(data.movies && data.movies.length > 0) {
                    ...
                    document.getElementById('detailMeta').innerText = `Views: ${activeMovieData.clicks} | Files: ${activeMovieData.files.length} | Category: ${activeMovieData.category}`;
                    const fRes = await fetch(`/api/fav/check/${uid}/${encodeURIComponent(activeMovieData._id)}`); const fData = await fRes.json();
                    isFav = fData.is_fav; updateFavBtn(); dlUnlocked = false; openModal('detailModal');
                }
            }

            function updateFavBtn() { const btn = document.getElementById('favBtn'); if(isFav) { btn.innerHTML = '<i class="fa-solid fa-heart" style="color:#ef4444;"></i> ফেভারিট থেকে সরান'; } else { btn.innerHTML = '<i class="fa-regular fa-heart"></i> ফেভারিটে যোগ করুন'; } }
            ...
            async function toggleFavorite() {
                ...
                const data = await res.json(); isFav = data.is_fav; updateFavBtn();
            }

            function startDownload() { if(isUserVip || dlUnlocked) { sendFile(); } else { startAdTimer(); } }

            function startAdTimer() {
                openModal('adModal'); let timeLeft = 15; document.getElementById('adLinkBtn').style.display = 'block'; document.getElementById('timerCircle').innerText = timeLeft;
                if(timerInterval) clearInterval(timerInterval);
                timerInterval = setInterval(() => { timeLeft--; document.getElementById('timerCircle').innerText = timeLeft; if(timeLeft <= 0) { clearInterval(timerInterval); dlUnlocked = true; closeModal('adModal'); sendFile(); } }, 1000);
            }

            function visitAdLink() { if(DIRECT_LINKS.length > 0) { tg.openLink(DIRECT_LINKS[Math.floor(Math.random() * DIRECT_LINKS.length)]); } }

            async function sendFile() {
                if(!activeMovieData) return; const fileId = activeMovieData.files[0].id;
                try {
                    ...
                    const data = await res.json(); if(data.ok) { tg.showAlert("✅ ফাইল বটের ইনবক্সে পাঠানো হয়েছে!"); closeModal('detailModal'); } else { tg.showAlert("⚠️ সমস্যা হয়েছে!"); }
                } catch(e) {}
            }

            async function loadFavorites() {
                const grid = document.getElementById('favGrid'); grid.innerHTML = '<div class="skeleton"></div>';
                try {
                    const r = await fetch(`/api/fav/list/${uid}`); const data = await r.json();
                    if(data.length > 0) { grid.innerHTML = data.map(m => `<div class="movie-row" onclick="openDetail('${m.title.replace(/'/g, "\\'")}')"><img class="movie-poster" src="/api/image/${m.photo_id}" onerror="this.src='https://via.placeholder.com/90x120'"><div class="movie-info"><div class="movie-title-text">${m.title}</div></div></div>`).join(''); }
                    else { grid.innerHTML = '<p style="text-align:center; color:#94a3b8; padding:20px;">কোনো ফেভারিট নেই!</p>'; }
                } catch(e) {}
            }

            async function loadUpcoming() {
                const grid = document.getElementById('upcomingGrid'); grid.innerHTML = '<div class="skeleton"></div>';
                try {
                    const r = await fetch('/api/upcoming'); const data = await r.json();
                    if(data.length > 0) { grid.innerHTML = data.map(m => `<div class="upc-card"><img class="upc-poster" src="/api/image/${m.photo_id}" onerror="this.src='https://via.placeholder.com/80x100'"><div class="upc-info"><div style="font-weight:700; margin-bottom:5px;">${m.title}</div><div style="font-size:12px; color:#00f260; margin-bottom:3px;"><i class="fa-solid fa-calendar"></i> ${m.release_date || 'N/A'}</div><div style="font-size:12px; color:#94a3b8;"><i class="fa-solid fa-language"></i> ${m.language || 'N/A'}</div></div></div>`).join(''); }
    ...
    """
    html_code = html_code.replace("{{DIRECT_LINKS}}", dl_json).replace("{{SOCIAL_JSON}}", social_json).replace("{{TG_LINK}}", tg_url)
    return html_code
...
async def get_user_info(uid: int):
    user = await db.users.find_one({"user_id": uid})
    if not user: return {"vip": False, "is_admin": False, "refer_count": 0, "coins": 0}
    now = datetime.datetime.utcnow()
    is_vip = user.get("vip_until", now) > now
    return {"vip": is_vip, "is_admin": uid in admin_cache, "refer_count": user.get("refer_count", 0), "coins": user.get("coins", 0)}
...
# Favorites APIs
class FavModel(BaseModel): uid: int; title: str; photo_id: str; initData: str
...
async def toggle_fav(data: FavModel):
    ...
    if existing: await db.favorites.delete_one({"_id": existing["_id"]}); return {"ok": True, "is_fav": False}
    else: await db.favorites.insert_one({"user_id": data.uid, "movie_title": data.title, "photo_id": data.photo_id}); return {"ok": True, "is_fav": True}
