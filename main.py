...
async def auto_lock_worker():
    while True:
        try:
            ...
            if result.deleted_count > 0: print(f"Auto-locked {result.deleted_count} movies.")
...
async def finish_category_selection(c: types.CallbackQuery, state: FSMContext):
    ...
    if LOG_CHANNEL_ID:
        try:
            log_kb = [[types.InlineKeyboardButton(text="🎬 Watch Now", url="https://t.me/MovieeBoxx_Bot?start=new")], [types.InlineKeyboardButton(text="🔴 18+ Channel", url=link_18)], [types.InlineKeyboardButton(text="📥 ডাউনলোড কিভাবে করবেন", url="https://t.me/SakibMovieBox/62")], [types.InlineKeyboardButton(text="📝 Request Movie", url="https://t.me/requestmoviebox")]]
...
# Add Upload Movie (New Quality for Existing Movie)
...
async def add_upload_movie_start(m: types.Message, state: FSMContext):
    ...
    await m.answer("📤 আপলোড মুভি অ্যাড করতে প্রথমে <b>মুভির নাম</b> লিখুন (যে মুভিতে নতুন কোয়ালিটি অ্যাড করতে চান)।", parse_mode="HTML")
...
async def receive_upload_movie_title(m: types.Message, state: FSMContext):
    ...
    if not movie:
        await m.answer("❌ এই নামের কোনো মুভি ডেটাবেসে নেই। আগে মুভি অ্যাড করুন।", parse_mode="HTML")
        await state.clear()
        return
    ...
    await m.answer("✅ মুভি পাওয়া গেছে! এবার <b>নতুন কোয়ালিটি</b> লিখুন (যেমন: 720p বা 1080p)।", parse_mode="HTML")
...
async def receive_upload_quality(m: types.Message, state: FSMContext):
    ...
    await m.answer("✅ এবার নতুন কোয়ালিটির <b>ভিডিও/ফাইল</b> পাঠান।", parse_mode="HTML")
...
async def receive_upload_file(m: types.Message, state: FSMContext):
    ...
    await m.answer(f"✅ <b>{data['title']} [{data['quality']}]</b> সফলভাবে যুক্ত হয়েছে!\n\n⚠️ এটি শুধু নতুন কোয়ালিটি তাই ব্রডকাস্ট পাঠানো হয়নি।", parse_mode="HTML")

@dp.message(AdminStates.waiting_for_upload_file)
async def fallback_upload_file(m: types.Message):
    await m.answer("⚠️ দয়া করে <b>ভিডিও/ডকুমেন্ট ফাইল</b> পাঠান। অথবা /cancel লিখুন।", parse_mode="HTML")
...
async def run_movie_broadcast(data, selected_cats, admin_id):
    ...
    bcast_kb = [[types.InlineKeyboardButton(text="🎬 Watch Now", web_app=types.WebAppInfo(url=web_app_url))], [types.InlineKeyboardButton(text="🚀 Join Channel", url=tg_link)], [types.InlineKeyboardButton(text="🔴 18+ Channel", url=link_18)], [types.InlineKeyboardButton(text="📥 ডাউনলোড কিভাবে করবেন", url="https://t.me/SakibMovieBox/62")], [types.InlineKeyboardButton(text="📝 Request Movie", url="https://t.me/requestmoviebox")]]
    ...
    try: await bot.send_message(admin_id, f"✅ <b>{data['title']}</b> এর ব্রডকাস্ট শেষ!\n\nসফলভাবে পাঠানো হয়েছে: <b>{bcast_success}</b> জনকে।", parse_mode="HTML")
    except: pass
...
async def admin_panel_ui(auth: bool = Depends(verify_admin)):
    ...
    <head>
        ...
        <style>
            ...
            table { width: 100%; border-collapse: collapse; min-width: 600px; } th { text-align: left; padding: 15px; color: #94a3b8; font-size: 12px; text-transform: uppercase; letter-spacing: 1px; border-bottom: 1px solid #334155; } td { padding: 15px; border-bottom: 1px solid #334155; font-size: 14px; color: #e2e8f0; } tr:last-child td { border-bottom: none; }
            ...
            .btn-group { display: flex; gap: 10px; flex-wrap: wrap; margin-bottom: 20px; justify-content: center; }
            .btn-admin { padding: 12px 20px; border-radius: 10px; border: none; font-weight: 700; cursor: pointer; color: #fff; text-decoration: none; display: flex; align-items: center; gap: 8px; }
    ...
    <body>
        ...
        <div class="btn-group">
            <a href="https://t.me/{BOT_USERNAME}" class="btn-admin btn-add" target="_blank"><i class="fa-solid fa-plus"></i> Add New Movie</a>
            <a href="https://t.me/{BOT_USERNAME}?start=addupload" class="btn-admin btn-upload" target="_blank"><i class="fa-solid fa-upload"></i> Add Upload Movie</a>
        </div>
        <div class="stats-grid">
            ...
            <div class="stat-card"><h3>Today's Users</h3><div class="value"><i class="fa-solid fa-user-plus" style="color:#10b981"></i> <span id="todayUsers">0</span></div></div>
            ...
            <div class="stat-card"><h3>Today's Clicks</h3><div class="value"><i class="fa-solid fa-chart-line" style="color:#ef4444"></i> <span id="todayClicks">0</span></div></div>
        </div>
        <div class="table-container"><div class="table-header"><h2><i class="fa-solid fa-film"></i> Uploaded Movies</h2></div><table><thead><tr><th>Title</th><th>Quality</th><th>Category</th><th>Views</th><th>Action</th></tr></thead><tbody id="movieTableBody"><tr><td colspan="5" class="empty-state">Loading...</td></tr></tbody></table></div>
        <script>
...
# Handle Add Upload Command from Bot Deep Linking
@dp.message(Command("start"), F.text.regexp(r"^/start addupload$"))
async def start_add_upload(m: types.Message, state: FSMContext):
    if m.from_user.id not in admin_cache: return
    await add_upload_movie_start(m, state)

@app.get("/api/trending")
async def get_trending_movies():
    seven_days_ago = datetime.datetime.utcnow() - datetime.timedelta(days=7)
    pipeline = [
        {"$match": {"unlocked_at": {"$gte": seven_days_ago}}},
        {"$group": {"_id": "$title", "count": {"$sum": 1}}},
        {"$sort": {"count": -1}},
        {"$limit": 5}
    ]
    trending_titles = await db.user_unlocks.aggregate(pipeline).to_list(5)
    titles = [t["_id"] for t in trending_titles]
    movies = await db.movies.find({"title": {"$in": titles}}).to_list(5)
    for m in movies: m["_id"] = str(m["_id"])
    return movies
...
async def get_movies(category: str = "All", search: str = "", page: int = 1):
    ...
    movies = await db.movies.find(query).sort("created_at", -1).skip(skip).limit(per_page).to_list(per_page)
    for m in movies: m["_id"] = str(m["_id"])
...
async def get_movie_detail(title: str):
    ...
    if not movies: raise HTTPException(status_code=404, detail="Movie not found")
    base = movies[0]
    ...
    <head>
        ...
        <style>
            ...
            * { margin: 0; padding: 0; box-sizing: border-box; }
            ...
            .section-title { font-size: 18px; font-weight: 800; margin-bottom: 10px; display: flex; align-items: center; gap: 8px; }
            .trending-slider { display: flex; overflow-x: auto; scroll-snap-type: x mandatory; gap: 10px; scrollbar-width: none; -ms-overflow-style: none; }
            .trending-slider::-webkit-scrollbar { display: none; }
            .trending-card { min-width: 85%; scroll-snap-align: start; border-radius: 12px; overflow: hidden; position: relative; cursor: pointer; flex-shrink: 0; }
            ...
            .trending-title { font-weight: 800; font-size: 16px; }
            
            /* Movie List */
            .movie-list { padding: 0 15px; display: grid; grid-template-columns: repeat(2, 1fr); gap: 10px; }
            .movie-card { background: rgba(30, 41, 59, 0.6); border-radius: 12px; overflow: hidden; border: 1px solid #334155; cursor: pointer; transition: 0.3s; position: relative; }
            ...
            .ad-title { font-size: 20px; font-weight: 800; margin-bottom: 15px; color: #fbbf24; }
            .ad-timer { font-size: 16px; color: #cbd5e1; margin-bottom: 15px; }
            ...
            .ad-btn-wait { background: #334155; color: #94a3b8; cursor: not-allowed; }
            .ad-btn-download { background: #10b981; color: white; display: none; }
            .ad-btn-skip { background: #ef4444; color: white; }
            .ad-link-btn { display: block; width: 100%; padding: 15px; border-radius: 10px; background: #ea580c; color: white; font-weight: 700; text-decoration: none; margin-bottom: 10px; text-align: center; }
        </style>
    </head>
    <body>
        ...
        <div id="tabProfile" class="page-section">
            <div class="profile-card">
                <h2 style="margin-bottom:15px">⚙️ Settings & Links</h2>
                <a href="https://t.me/addlist/MwbWNafSFK4yZjhl" class="ad-link-btn" style="background:#24A1DE">🚀 Join Channel</a>
                <a href="https://t.me/+W5V9-mn08jMyYTE1" class="ad-link-btn" style="background:#ef4444">🔴 18+ Channel</a>
                <button onclick="toggleDarkMode()" class="ad-link-btn" style="background:#334155">🌓 Toggle OLED Mode</button>
            </div>
        </div>
        ...
        <div class="ad-overlay" id="adModal">
            <div class="ad-content">
                ...
                <div class="ad-title">বিজ্ঞাপন</div>
                <div class="ad-timer" id="adTimerText">অনুগ্রহ করে ১০ সেকেন্ড অপেক্ষা করুন...</div>
                <a id="adVisitLink" href="#" class="ad-link-btn" target="_blank">বিজ্ঞাপন দেখুন</a>
                <button id="adWaitBtn" class="ad-btn ad-btn-wait" disabled>১০ সেকেন্ড অপেক্ষা করুন</button>
                <button id="adDownloadBtn" class="ad-btn ad-btn-download" onclick="finalDownload()">ডাউনলোড করুন</button>
            </div>
        </div>
        ...
        <script>
            ...
            tg.ready(); tg.expand();
            const DIRECT_LINKS = ''' + dl_json + ''';
            const ADULT_LINKS = ''' + adl_json + ''';
            const FILE_BASE = "https://api.telegram.org/file/bot''' + TOKEN + '''/";
            
            let currentCat = "All";
            let currentPage = 1;
            let searchQuery = "";
            let currentUser = tg.initDataUnsafe && tg.initDataUnsafe.user ? tg.initDataUnsafe.user : {id: 0, first_name: "Guest"};
            let currentFileUrl = "";
            let adInterval = null;
            ...
            function switchTab(tab) {
                ...
                if(tab === 'home') { document.getElementById('tabHome').classList.add('active'); document.querySelectorAll('.nav-item')[0].classList.add('active'); }
                else { document.getElementById('tabProfile').classList.add('active'); document.querySelectorAll('.nav-item')[1].classList.add('active'); }
            }

            function toggleDarkMode() { document.body.classList.toggle('oled-mode'); localStorage.setItem('oled', document.body.classList.contains('oled-mode')); }
            ...
            async function loadCategories() {
                ...
                row.innerHTML = cats.map(c => `<div class="cat-chip ${c === currentCat ? 'active' : ''}" onclick="selectCat('${c}')">${c}</div>`).join('');
            }
            ...
            async function loadTrending() {
                try {
                    ...
                    if(movies.length > 0) {
                        ...
                        slider.innerHTML = movies.map(m => `<div class="trending-card" onclick="openMovie('${m.title}')"><img src="${FILE_BASE}${m.photo_id}" onerror="this.src='https://via.placeholder.com/16:9'"><div class="trending-overlay"><div class="trending-title">${m.title}</div></div></div>`).join('');
                        startAutoSlider();
                    } else { document.getElementById('trendingSection').style.display = 'none'; }
            ...
            async function loadMovies() {
                try {
                    ...
                    list.innerHTML = data.movies.map(m => `<div class="movie-card" onclick="openMovie('${m.title}')"><img src="${FILE_BASE}${m.photo_id}" onerror="this.src='https://via.placeholder.com/16:9'"><div class="movie-info"><div class="movie-title">${m.title}</div><div class="movie-meta"><span>${m.year || 'N/A'}</span><span>${m.quality || ''}</span></div></div></div>`).join('');
            ...
            function renderPagination(totalPages, current) {
                ...
                let html = `<button class="page-btn" onclick="goToPage(${current-1})" ${current === 1 ? 'disabled' : ''}><i class="fa-solid fa-chevron-left"></i></button>`;
                for(let i=1; i<=totalPages; i++) { if(i === current || i === 1 || i === totalPages || Math.abs(i - current) <= 1) html += `<button class="page-btn ${i === current ? 'active' : ''}" onclick="goToPage(${i})">${i}</button>`; else if(Math.abs(i - current) === 2) html += `<span style="color:#64748b">...</span>`; }
                html += `<button class="page-btn" onclick="goToPage(${current+1})" ${current === totalPages ? 'disabled' : ''}><i class="fa-solid fa-chevron-right"></i></button>`;
                pag.innerHTML = html;
            }
            ...
            async function openMovie(title) {
                try {
                    ...
                    fetch(`/api/movie/click/${encodeURIComponent(title)}`);
                    document.getElementById('detailImg').src = FILE_BASE + m.photo_id;
                    document.getElementById('detailTitle').innerText = m.title;
                    document.getElementById('detailMeta').innerText = `📅 ${m.year} | 👁️ ${m.clicks} Views`;
                    
                    let qHtml = '';
                    m.qualities.forEach(q => {
                        const isAdult = m.categories.includes("Adult Content");
                        qHtml += `<button class="dl-file-btn" onclick="initDownload('${q.file_id}', '${q.file_type}', ${isAdult})"><span><i class="fa-solid fa-download"></i> ${q.quality}</span><i class="fa-solid fa-arrow-right"></i></button>`;
                    });
                    document.getElementById('detailQualities').innerHTML = qHtml;
                    document.getElementById('detailModal').style.display = 'flex';

                    if(currentUser.id) {
                        fetch('/api/unlock', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({user_id: currentUser.id, title: m.title}) });
                    }
            ...
            function initDownload(fileId, fileType, isAdult) {
                if(fileType === 'video') currentFileUrl = `https://api.telegram.org/bot/''' + TOKEN + '''/getFile?file_id=${fileId}`;
                else currentFileUrl = `https://api.telegram.org/bot/''' + TOKEN + '''/getFile?file_id=${fileId}`;
                
                fetch(currentFileUrl).then(r => r.json()).then(d => {
                    if(d.ok) currentFileUrl = FILE_BASE + d.result.file_path;
                    else currentFileUrl = "";
                }).catch(() => currentFileUrl = "");

                const links = isAdult ? ADULT_LINKS : DIRECT_LINKS;
                const randomLink = links.length > 0 ? links[Math.floor(Math.random() * links.length)] : "#";
                
                document.getElementById('adVisitLink').href = randomLink;
                document.getElementById('adDownloadBtn').style.display = 'none';
                document.getElementById('adWaitBtn').style.display = 'block';
                document.getElementById('adWaitBtn').disabled = true;
                document.getElementById('adTimerText').innerText = "অনুগ্রহ করে ১০ সেকেন্ড অপেক্ষা করুন...";
                ...
                adInterval = setInterval(() => {
                    adSecondsLeft--;
                    if(adSecondsLeft > 0) {
                        document.getElementById('adTimerText').innerText = `অনুগ্রহ করে ${adSecondsLeft} সেকেন্ড অপেক্ষা করুন...`;
                        document.getElementById('adWaitBtn').innerText = `${adSecondsLeft} সেকেন্ড অপেক্ষা করুন`;
                    } else {
                        clearInterval(adInterval);
                        document.getElementById('adTimerText').innerText = "আপনি এখন ডাউনলোড করতে পারেন!";
            ...
            function finalDownload() {
                ...
                else alert("ডাউনলোড লিংক তৈরি হতে সমস্যা হয়েছে।");
