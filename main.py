...
async def start_cmd(message: types.Message, state: FSMContext):
    ...
    # Deep link for Add Upload Movie
    if uid in admin_cache and len(args) > 1 and args[1] == "uploadmovie":
        await state.set_state(AdminStates.waiting_for_upload_title_select)
        return await message.answer("✅ এবার <b>যে মুভিতে নতুন কোয়ালিটি যোগ করতে চান তার নাম</b> লিখুন:", parse_mode="HTML")

    # Deep link for Downloading file
                ...
                if quality and "qualities" in movie:
                    for q in movie["qualities"]:
                        if q.get("quality") == quality:
                            ...
                            break
                if not target_file:
                    target_file = {"file_id": movie.get("file_id"), "file_type": movie.get("file_type"), "quality": movie.get("quality")}
                
                if target_file and target_file.get("file_id"):
                    await bot.send_chat_action(uid, "upload_video")
                    caption = f"🎬 {movie['title']} [{target_file.get('quality', movie.get('quality'))}]"
                    if target_file.get("file_type") == "video":
                        await bot.send_video(uid, video=target_file["file_id"], caption=caption)
                    else:
                        await bot.send_document(uid, document=target_file["file_id"], caption=caption)
                    return
        except:
            pass
        await message.answer("❌ মুভি পাওয়া যায়নি বা ডাউনলোড এরর!")
        return
...
# ==========================================
# 7.5 Add New Movie (Single File Upload)
# ==========================================
...
async def receive_movie_file(m: types.Message, state: FSMContext):
    ...
    if current_state == AdminStates.waiting_for_upload_file:
        ...
        if not movie_doc:
            return await m.answer("❌ মুভি পাওয়া যায়নি!", parse_mode="HTML")
            
        await db.movies.update_one({"title": movie_title}, {"$push": {"qualities": {"quality": quality, "file_id": fid, "file_type": ftype}}})
        return await m.answer(f"✅ <b>{movie_title}</b> মুভিতে নতুন কোয়ালিটি <b>{quality}</b> যুক্ত হয়েছে!\n\n⚠️ শুধু কোয়ালিটি যোগ করা হয়েছে তাই কোনো ব্রডকাস্ট যাবে না.", parse_mode="HTML")
...
async def finish_category_selection(c: types.CallbackQuery, state: FSMContext):
    ...
    # Broadcast will run only for NEW MOVIES
...
async def upload_movie_start(m: types.Message, state: FSMContext):
    if m.from_user.id not in admin_cache: return
    await state.set_state(AdminStates.waiting_for_upload_title_select)
    await m.answer("✅ এবার <b>যে মুভিতে নতুন কোয়ালিটি যোগ করতে চান তার নাম</b> লিখুন:", parse_mode="HTML")
...
async def upload_movie_title(m: types.Message, state: FSMContext):
    ...
    if not movie:
        await state.clear()
        return await m.answer("❌ এই নামে কোনো মুভি পাওয়া যায়নি!", parse_mode="HTML")
    
    await state.update_data(upload_title=movie["title"])
    await state.set_state(AdminStates.waiting_for_upload_file)
    await m.answer(f"✅ মুভি পাওয়া গেছে: <b>{movie['title']}</b>\n\nএবার নতুন কোয়ালিটির ফাইল (ভিডিও/ডকুমেন্ট) পাঠান.\n⚠️ কোয়ালিটি নাম লিখতে ক্যাপশন ব্যবহার করুন (যেমন: 720p)", parse_mode="HTML")
...
# ==========================================
# 8. Image Proxy Route (Fixes Poster Loading)
# ==========================================
@app.get("/api/poster/{file_id}")
async def proxy_poster(file_id: str):
    try:
    ...
    <head>
        ...
        <style>
            ...
            * { margin: 0; padding: 0; box-sizing: border-box; }
            ...
            .trending-section { margin: 0 15px 20px 15px; }
            .trending-title { padding: 0 0 10px; font-size: 18px; font-weight: 800; color: #fbbf24; display: flex; align-items: center; gap: 8px; }
            .trending-slider { position: relative; overflow: hidden; border-radius: 12px; }
