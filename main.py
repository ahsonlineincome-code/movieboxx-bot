...
def validate_tg_data(init_data: str) -> bool:
    """
    BUG FIX: hmac.new() does not exist in Python.
    Correct API is hmac.new(key, msg, digestmod).
    Also fixed parameter order for secret_key generation.
    """
    try:
...
async def start_cmd(message: types.Message, state: FSMContext):
    ...
    if uid in banned_cache:
        return await message.answer("🚫 <b>আপনাকে এই বট থেকে স্থায়ীভাবে ব্যান করা হয়েছে।</b>", parse_mode="HTML")
                    ...
                    if ref_user and ref_user.get("refer_count", 0) % 5 == 0:
                        ...
                        try:
                            await bot.send_message(
                                referrer_id,
                                "🎉 <b>অভিনন্দন!</b> আপনার ৫ জন রেফার পূর্ণ হয়েছে। আপনাকে ২৪ ঘণ্টার জন্য <b>VIP</b> দেওয়া হয়েছে! এখন আপনি বিনা অ্যাডে মুভি ডাউনলোড করতে পারবেন।",
                                parse_mode="HTML"
                            )
                        except:
                            pass
    ...
    if uid in admin_cache:
        text = (
            ...
            f"🌐 <b>ওয়েব অ্যাডমিন প্যানেল:</b> <a href='{APP_URL}/admin'>এখানে ক্লিক করুন</a>\n"
...
# BUG FIX: Added check so admin messages don't get forwarded to owner
...
async def forward_to_admin(m: types.Message):
    try:
        ...
        await bot.send_message(
            OWNER_ID,
            f"📩 <b>New Message from <a href='tg://user?id={m.from_user.id}'>{m.from_user.first_name}</a></b>:\n\n{m.text or 'Media file'}",
            parse_mode="HTML",
...
async def add_direct_link(m: types.Message):
    ...
    try:
        ...
        await m.answer(f"✅ ডাইরেক্ট লিংক সফলভাবে অ্যাড করা হয়েছে:\n<code>{url}</code>", parse_mode="HTML")
    except Exception:
        await m.answer("⚠️ সঠিক নিয়ম: <code>/addlink https://example.com</code>", parse_mode="HTML")
...
async def del_direct_link(m: types.Message):
    ...
    try:
        ...
        if result.modified_count > 0:
            await m.answer(f"❌ লিংকটি সফলভাবে ডিলিট করা হয়েছে:\n<code>{url}</code>", parse_mode="HTML")
        else:
            await m.answer("⚠️ লিংকটি ডাটাবেসে পাওয়া যায়নি।")
    except Exception:
        await m.answer("⚠️ সঠিক নিয়ম: <code>/dellink https://example.com</code>", parse_mode="HTML")
...
async def add_admin_cmd(m: types.Message):
    if m.from_user.id != OWNER_ID:
        return await m.answer("⚠️ শুধুমাত্র মেইন Owner অ্যাডমিন অ্যাড করতে পারবে!")
    try:
        ...
        await m.answer(f"✅ ইউজার <code>{target_uid}</code> কে সফলভাবে অ্যাডমিন বানানো হয়েছে!", parse_mode="HTML")
    except Exception:
        await m.answer("⚠️ সঠিক নিয়ম: <code>/addadmin ইউজার_আইডি</code>", parse_mode="HTML")


@dp.message(Command("deladmin"))
async def del_admin_cmd(m: types.Message):
    if m.from_user.id != OWNER_ID:
        return await m.answer("⚠️ শুধুমাত্র মেইন Owner অ্যাডমিন রিমুভ করতে পারবে!")
    try:
        target_uid = int(m.text.split()[1])
        if target_uid == OWNER_ID:
            return await m.answer("⚠️ Main Owner কে ডিলিট করা সম্ভব নয়!")
        await db.admins.delete_one({"user_id": target_uid})
        admin_cache.discard(target_uid)
        await m.answer(f"❌ ইউজার <code>{target_uid}</code> কে অ্যাডমিন লিস্ট থেকে রিমুভ করা হয়েছে!", parse_mode="HTML")
    except Exception:
        await m.answer("⚠️ সঠিক নিয়ম: <code>/deladmin ইউজার_আইডি</code>", parse_mode="HTML")
...
async def del_movie_cmd(m: types.Message):
    ...
    try:
        ...
        if result.deleted_count > 0:
            await m.answer(f"✅ '<b>{title}</b>' নামের {result.deleted_count} টি ফাইল সফলভাবে ডিলিট হয়েছে!", parse_mode="HTML")
        else:
            await m.answer("⚠️ এই নামের কোনো মুভি ডাটাবেসে পাওয়া যায়নি। (নাম হুবহু মিলতে হবে)")
    except Exception:
        await m.answer("⚠️ সঠিক নিয়ম: <code>/delmovie মুভির সম্পূর্ণ নাম</code>", parse_mode="HTML")
...
async def stats_cmd(m: types.Message):
    ...
    top_movies_text = "".join(
        f"{idx}. {mv['_id'][:20]}... - <b>{format_views(mv['clicks'])} views</b>\n"
...
async def ban_user_cmd(m: types.Message):
    ...
    try:
        ...
        await m.answer(f"🚫 ইউজার <code>{target_uid}</code> কে ব্যান করা হয়েছে!", parse_mode="HTML")
    except Exception:
        await m.answer("⚠️ সঠিক নিয়ম: <code>/ban ইউজার_আইডি</code>", parse_mode="HTML")
...
async def unban_user_cmd(m: types.Message):
    ...
    try:
        ...
        await m.answer(f"✅ ইউজার <code>{target_uid}</code> আনব্যান হয়েছে!", parse_mode="HTML")
    except Exception:
        await m.answer("⚠️ সঠিক নিয়ম: <code>/unban ইউজার_আইডি</code>", parse_mode="HTML")
...
async def set_ad_count_cmd(m: types.Message):
    ...
    try:
        ...
        await m.answer(f"✅ অ্যাড দেখার সংখ্যা সেট করা হয়েছে: <b>{count} টি</b>।", parse_mode="HTML")
    except Exception:
        await m.answer("⚠️ সঠিক নিয়ম: <code>/setadcount 3</code>", parse_mode="HTML")
...
async def protect_cmd(m: types.Message):
    if m.from_user.id not in admin_cache:
        return
    try:
        state_val = m.text.split(" ")[1].lower()
        if state_val == "on":
            await db.settings.update_one({"id": "protect_content"}, {"$set": {"status": True}}, upsert=True)
            await m.answer("✅ ফরোয়ার্ড প্রোটেকশন চালু করা হয়েছে।")
        elif state_val == "off":
            await db.settings.update_one({"id": "protect_content"}, {"$set": {"status": False}}, upsert=True)
            await m.answer("✅ ফরোয়ার্ড প্রোটেকশন বন্ধ করা হয়েছে।")
        else:
            await m.answer("⚠️ সঠিক নিয়ম: <code>/protect on</code> অথবা <code>/protect off</code>", parse_mode="HTML")
    except Exception:
        await m.answer("⚠️ সঠিক নিয়ম: <code>/protect on</code> অথবা <code>/protect off</code>", parse_mode="HTML")
...
async def set_del_time(m: types.Message):
    ...
    try:
        ...
        await m.answer(f"✅ অটো-ডিলিট টাইম সেট করা হয়েছে: <b>{mins} মিনিট</b>।", parse_mode="HTML")
    except Exception:
        await m.answer("⚠️ সঠিক নিয়ম: <code>/settime 60</code> (মিনিট)", parse_mode="HTML")
...
async def set_ad(m: types.Message):
    ...
    try:
        ...
        await m.answer(f"✅ জোন আপডেট হয়েছে: <b>{zone}</b>", parse_mode="HTML")
    except Exception:
        await m.answer("⚠️ সঠিক নিয়ম: <code>/setad 1234567</code>", parse_mode="HTML")
...
async def set_tg_link(m: types.Message):
    ...
    try:
        ...
        await m.answer(f"✅ টেলিগ্রাম লিংক সেট করা হয়েছে: <b>{link}</b>", parse_mode="HTML")
    except Exception:
        await m.answer("⚠️ সঠিক নিয়ম: <code>/settg https://t.me/YourChannel</code>", parse_mode="HTML")
...
async def set_18_link(m: types.Message):
    ...
    try:
        ...
        await m.answer(f"✅ 18+ লিংক সেট করা হয়েছে: <b>{link}</b>", parse_mode="HTML")
    except Exception:
        await m.answer("⚠️ সঠিক নিয়ম: <code>/set18 https://t.me/YourChannel</code>", parse_mode="HTML")
...
async def set_bkash(m: types.Message):
    ...
    try:
        ...
        await m.answer(f"✅ বিকাশ নাম্বার সেট করা হয়েছে: <b>{num}</b>", parse_mode="HTML")
    except Exception:
        await m.answer("⚠️ সঠিক নিয়ম: <code>/setbkash 017XXXXXXXX</code>", parse_mode="HTML")
...
async def set_nagad(m: types.Message):
    ...
    try:
        ...
        await m.answer(f"✅ নগদ নাম্বার সেট করা হয়েছে: <b>{num}</b>", parse_mode="HTML")
    except Exception:
        await m.answer("⚠️ সঠিক নিয়ম: <code>/setnagad 017XXXXXXXX</code>", parse_mode="HTML")
...
async def add_vip_cmd(m: types.Message):
    ...
    try:
        ...
        await m.answer(f"✅ ইউজার <code>{target_uid}</code> কে সফলভাবে <b>{days} দিনের</b> VIP দেওয়া হয়েছে!", parse_mode="HTML")

        try:
            await bot.send_message(
                target_uid,
                f"🎉 <b>অভিনন্দন!</b> অ্যাডমিন আপনাকে <b>{days} দিনের</b> জন্য VIP মেম্বারশিপ দিয়েছেন।\n\nএখন আপনি কোনো অ্যাড ছাড়াই মুভি ডাউনলোড করতে পারবেন এবং আপনার ফাইল কখনো অটো-ডিলিট হবে না!",
                parse_mode="HTML"
            )
        except Exception:
            pass
    except Exception:
        await m.answer("⚠️ সঠিক নিয়ম: <code>/addvip ইউজার_আইডি দিন</code>\nউদাহরণ: <code>/addvip 123456789 30</code>", parse_mode="HTML")
...
async def remove_vip_cmd(m: types.Message):
    ...
    try:
        ...
        await m.answer(f"❌ ইউজার <code>{target_uid}</code> এর VIP বাতিল করা হয়েছে!", parse_mode="HTML")
    except Exception:
        await m.answer("⚠️ সঠিক নিয়ম: <code>/removevip ইউজার_আইডি</code>", parse_mode="HTML")
...
async def handle_trx_approval(c: types.CallbackQuery):
    ...
    # Format: trx_approve_<id> or trx_reject_<id>
    action = parts[1]   # "approve" or "reject"
    pay_id = parts[2]

    payment = await db.payments.find_one({"_id": ObjectId(pay_id)})
    if not payment or payment["status"] != "pending":
        return await c.answer("⚠️ এই পেমেন্টটি ইতিমধ্যে প্রসেস করা হয়েছে!", show_alert=True)
    ...
    if action == "approve":
        ...
        await c.message.edit_text(c.message.text + f"\n\n✅ <b>পেমেন্ট অ্যাপ্রুভ করা হয়েছে! ({pkg_text})</b>", parse_mode="HTML")
        try:
            await bot.send_message(user_id, f"🎉 <b>পেমেন্ট সফল!</b> আপনার পেমেন্ট অ্যাপ্রুভ হয়েছে এবং আপনাকে <b>{pkg_text}</b> VIP দেওয়া হয়েছে!", parse_mode="HTML")
        except:
            pass
    else:
        await db.payments.update_one({"_id": ObjectId(pay_id)}, {"$set": {"status": "rejected"}})
        await c.message.edit_text(c.message.text + "\n\n❌ <b>পেমেন্ট রিজেক্ট করা হয়েছে!</b>", parse_mode="HTML")
        try:
            await bot.send_message(user_id, f"❌ <b>দুঃখিত!</b> আপনার পেমেন্ট (TrxID: {payment['trx_id']}) বাতিল করা হয়েছে। তথ্যে ভুল থাকলে সাপোর্ট অ্যাডমিনের সাথে যোগাযোগ করুন।", parse_mode="HTML")
        except:
            pass
...
async def handle_request_approval(c: types.CallbackQuery):
    ...
    if not req:
        return await c.answer("⚠️ রিকোয়েস্টটি ইতিমধ্যে ডিলিট বা প্রসেস করা হয়েছে!", show_alert=True)

    movie_name = req["movie"]
    voters = req.get("voters", [])

    if action == "acc":
        await c.message.edit_text(c.message.text + "\n\n✅ <b>Approve করা হয়েছে! (ইউজারদের জানানো হয়েছে)</b>", parse_mode="HTML")
        for v_id in voters:
            try:
                await bot.send_message(v_id, f"🎉 <b>সুখবর!</b> আপনার রিকোয়েস্ট করা/ভোট দেওয়া মুভি <b>{movie_name}</b> অ্যাপে আপলোড করা হয়েছে! এখনই অ্যাপ ওপেন করে দেখে নিন।", parse_mode="HTML")
            except:
                pass
        await db.requests.delete_one({"_id": ObjectId(req_id)})
    elif action == "rej":
        await c.message.edit_text(c.message.text + "\n\n❌ <b>Reject করা হয়েছে!</b>", parse_mode="HTML")
        for v_id in voters:
            try:
                await bot.send_message(v_id, f"❌ <b>দুঃখিত!</b> আপনার রিকোয়েস্ট করা মুভি <b>{movie_name}</b> এই মুহূর্তে আপলোড করা সম্ভব হচ্ছে না (হয়তো কোয়ালিটি ভালো না বা পাওয়া যায়নি)।", parse_mode="HTML")
            except:
                pass
        await db.requests.delete_one({"_id": ObjectId(req_id)})
...
async def receive_movie_file(m: types.Message, state: FSMContext):
    ...
    await m.answer("✅ ফাইল পেয়েছি! এবার মুভির <b>পোস্টার (Photo)</b> সেন্ড করুন।\nবাতিল করতে /start দিন।", parse_mode="HTML")
...
async def receive_movie_photo(m: types.Message, state: FSMContext):
    ...
    await m.answer(
        "✅ পোস্টার পেয়েছি! এবার <b>মুভি বা ওয়েব সিরিজের নাম</b> লিখে পাঠান।\n"
        "<i>(নোট: যদি ওয়েব সিরিজ হয় বা একই মুভির অন্য কোয়ালিটি অ্যাড করতে চান, তবে আগের নামটিই হুবহু দিন)</i>",
        parse_mode="HTML"
    )
...
async def receive_movie_title(m: types.Message, state: FSMContext):
    ...
    await m.answer(
        "✅ নাম সেভ হয়েছে! এবার এই ফাইলটির <b>কোয়ালিটি বা এপিসোড নাম্বার</b> দিন।\n"
        "<i>(উদাহরণ: 480p, 720p, 1080p অথবা Episode 01)</i>",
        parse_mode="HTML"
    )
...
async def receive_movie_quality(m: types.Message, state: FSMContext):
    ...
    await m.answer(f"🎉 <b>{title} [{quality}]</b> অ্যাপে সফলভাবে যুক্ত করা হয়েছে!", parse_mode="HTML")
    ...
    if req:
        for v_id in req.get("voters", []):
            try:
                await bot.send_message(
                    v_id,
                    f"🔔 <b>কাস্টম নোটিফিকেশন:</b>\n\n🎉 সুখবর! আপনার রিকোয়েস্ট করা/ভোট দেওয়া মুভি <b>{title}</b> অ্যাপে আপলোড করা হয়েছে! এখনই অ্যাপ ওপেন করে দেখে নিন।",
                    parse_mode="HTML"
                )
            except Exception:
                pass
    ...
    if CHANNEL_ID and CHANNEL_ID != "-100XXXXXXXXXX":
        try:
            ...
            markup = types.InlineKeyboardMarkup(inline_keyboard=kb)
            caption = (
                f"🎬 <b>নতুন ফাইল যুক্ত হয়েছে!</b>\n\n"
                f"📌 <b>নাম:</b> {title}\n"
                f"🏷 <b>কোয়ালিটি/এপিসোড:</b> {quality}\n\n"
...
async def upc_photo_step(m: types.Message, state: FSMContext):
    ...
    await m.answer("✅ পোস্টার পেয়েছি! এবার আপকামিং মুভির <b>টাইটেল (নাম)</b> লিখে পাঠান।", parse_mode="HTML")
...
async def upc_title_step(m: types.Message, state: FSMContext):
    ...
    await state.clear()
    await m.answer("✅ আপকামিং মুভি সফলভাবে যুক্ত করা হয়েছে!")
...
async def del_upc_cmd(m: types.Message):
    ...
    await m.answer("🗑 সব আপকামিং মুভি ডিলিট করা হয়েছে!")
...
async def execute_broadcast(m: types.Message, state: FSMContext):
    await state.clear()
    await m.answer("⏳ ব্রডকাস্ট শুরু হয়েছে...")
    ...
    async for u in db.users.find():
        try:
            ...
            pass
    await m.answer(f"✅ সম্পন্ন! সর্বমোট <b>{success}</b> জনকে মেসেজ পাঠানো হয়েছে।", parse_mode="HTML")
...
async def send_reply(m: types.Message, state: FSMContext):
    ...
    try:
        ...
        await m.answer("✅ ইউজারকে সফলভাবে রিপ্লাই পাঠানো হয়েছে!")
    except Exception:
        await m.answer("⚠️ রিপ্লাই পাঠানো যায়নি! ইউজার হয়তো বট ব্লক করেছে।")
    ...
    <body class="bg-gray-900 text-white font-sans antialiased relative">
        ...
        <div id="adminEditModal" class="fixed inset-0 bg-black bg-opacity-80 hidden flex items-center justify-center z-50">
            <div class="bg-gray-800 p-6 rounded-xl border border-gray-700 w-full max-w-md relative">
                ...
                <div class="mb-5">
                    <label class="block text-gray-400 text-sm mb-1 font-bold">Movie Title</label>
        ...
        <script>
            ...
            async function addViews(encodedTitle) {
                let amount = prompt("এই মুভির ভিউ কত বাড়াতে চান? (যেমন: 1000 বা 5000):", "1000");
                if (amount && amount.trim() !== "" && !isNaN(amount)) {
                    await fetch('/api/admin/movie/' + encodedTitle, {
                        method: 'PUT',
                        headers: {'Content-Type': 'application/json'},
                        body: JSON.stringify({add_clicks: parseInt(amount)})
                    });
                    loadAdminData();
                }
            }
...
async def delete_movie_api(title: str, auth: bool = Depends(verify_admin)):
    ...
    await db.movies.delete_many({"title": decoded_title})
    return {"ok": True}
...
async def edit_movie_api(title: str, data: dict = Body(...), auth: bool = Depends(verify_admin)):
    ...
    if update_data:
        await db.movies.update_many({"title": decoded_title}, {"$set": update_data})

    if add_clicks := data.get("add_clicks"):
        try:
            clicks_to_add = int(add_clicks)
            final_title = update_data.get("title", decoded_title)
            await db.movies.update_many({"title": final_title}, {"$inc": {"clicks": clicks_to_add}})
...
async def web_ui():
    ...
    <head>
        ...
        <style>
            * { margin: 0; padding: 0; box-sizing: border-box; }
            ...
            /* Splash Screen */
            #splash-screen { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: #020617; z-index: 999999; display: flex; justify-content: center; align-items: center; flex-direction: column; overflow: hidden; }
            .splash-bg { position: absolute; width: 500px; height: 500px; background: radial-gradient(circle, #ff00ff33, #00ffff11, transparent); filter: blur(40px); animation: bgMove 6s linear infinite alternate; }
            @keyframes bgMove { 0% { transform: translate(-50px,-50px) scale(1); } 100% { transform: translate(50px,50px) scale(1.3); } }
            .logo-wrapper { position: relative; display: flex; justify-content: center; align-items: center; flex-direction: column; z-index: 10; }
            .logo-ring { position: absolute; width: 220px; height: 220px; border-radius: 50%; border: 3px solid #00ffff; box-shadow: 0 0 20px #00ffff, 0 0 50px #00ffff, inset 0 0 20px #00ffff; animation: rotateRing 8s linear infinite; }
            .logo-ring::before { content: ""; position: absolute; inset: -10px; border-radius: 50%; border: 2px solid #ff00ff; box-shadow: 0 0 20px #ff00ff, 0 0 40px #ff00ff; }
            @keyframes rotateRing { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }
            .logo-text { font-size: 52px; font-weight: 900; letter-spacing: 3px; text-transform: uppercase; background: linear-gradient(45deg, #ff00ff, #00ffff, #ffffff, #ff00ff); background-size: 400%; -webkit-background-clip: text; -webkit-text-fill-color: transparent; animation: textGlow 5s linear infinite; }
            @keyframes textGlow { 0% { background-position: 0% 50%; transform: scale(0.8); opacity: 0; } 50% { transform: scale(1.05); opacity: 1; } 100% { background-position: 100% 50%; transform: scale(1); opacity: 1; } }
            .loading-text { margin-top: 30px; color: #94a3b8; font-size: 15px; letter-spacing: 4px; animation: blink 1.2s infinite; }
            @keyframes blink { 0%, 100% { opacity: 0.3; } 50% { opacity: 1; } }
            .particles { position: absolute; width: 6px; height: 6px; background: #00ffff; border-radius: 50%; box-shadow: 0 0 10px #00ffff; animation: floatParticle 8s linear infinite; }
            ...
            @keyframes floatParticle { 0% { transform: translateY(0px) scale(1); opacity: 0; } 50% { opacity: 1; } 100% { transform: translateY(-120px) scale(1.8); opacity: 0; } }
            .hide-splash { animation: hideSplash 1s forwards; }
            @keyframes hideSplash { to { opacity: 0; visibility: hidden; } }
        </style>
    </head>
    <body onclick="closeMenu(event)">

        <!-- BUG FIX: Splash screen HTML was missing from original code -->
        ...
        <div id="dropdownMenu" class="dropdown-menu">
            ...
            <a onclick="openReqModal()"><i class="fa-solid fa-code-pull-request text-green-400"></i> রিকোয়েস্ট ও ভোট 🗳️</a>
        </div>

        <div class="search-box">
            <input type="text" id="searchInput" class="search-input" placeholder="🔍 মুভি বা ওয়েব সিরিজ খুঁজুন...">
        </div>
        ...
        <div id="checkinModal" class="modal">
            <div class="modal-content">
                ...
                <i class="fa-solid fa-gift" style="font-size:70px; color:#ec4899;"></i>
                <h2 style="margin:15px 0 10px; color:white; font-size: 24px;">ডেইলি রিওয়ার্ড</h2>
                <p style="color:#cbd5e1; font-size:15px; line-height: 1.5;">প্রতিদিন লগিন করে ফ্রী কয়েন সংগ্রহ করুন। ৫০ কয়েন জমিয়ে ১ দিনের VIP কেনা যাবে!</p>
                ...
                <button class="btn-submit" style="background: linear-gradient(45deg, #3b82f6, #2563eb); margin-bottom: 10px;" onclick="claimCheckin()">আজকের ১০ কয়েন সংগ্রহ করুন</button>
                <button class="btn-submit" style="background: linear-gradient(45deg, #8b5cf6, #6d28d9); margin-bottom: 10px;" onclick="watchAdForCoins()"><i class="fa-solid fa-link"></i> ডাইরেক্ট লিংক ভিজিট করে ৫ কয়েন আয় করুন</button>
                <button class="btn-submit" style="background: linear-gradient(45deg, #f59e0b, #d97706); color:black;" onclick="convertCoins()"><i class="fa-solid fa-crown"></i> কয়েন দিয়ে VIP কিনুন (50 কয়েন)</button>
            </div>
        </div>
        ...
        <div id="vipModal" class="modal">
            <div class="modal-content">
                ...
                <div id="payBox" class="pay-box">
                    ...
                    <button class="btn-submit" style="margin-top:10px;" onclick="submitPayment()">পেমেন্ট ভেরিফাই করুন</button>
                </div>
            </div>
        </div>
        ...
        <div id="reqModal" class="modal">
            <div class="modal-content" style="max-height: 90vh;">
                ...
                <h2 style="color:white; font-size: 24px;">মুভি রিকোয়েস্ট ও ভোট 🗳️</h2>
                ...
                <p style="text-align:left; color:#94a3b8; font-size:14px; margin-top:15px; font-weight:bold;">ট্রেন্ডিং রিকোয়েস্ট:</p>
        ...
        <div id="chatModal" class="modal">
            <div class="modal-content" style="max-height: 90vh; display:flex; flex-direction:column;">
                ...
                <p style="color:#94a3b8; font-size:13px; margin-bottom:5px;">সবার সাথে কথা বলুন, মুভির নাম শেয়ার করুন!</p>
        ...
        <div id="spinModal" class="modal">
            <div class="modal-content">
                ...
                <p style="color:#cbd5e1; font-size:14px;">প্রতিদিন ৩ বার স্পিন করে ফ্রী কয়েন জিতে নিন! স্পিন করতে একটি লিংক ভিজিট করতে হবে।</p>
        ...
        <div id="tasksModal" class="modal">
            <div class="modal-content">
                ...
                <p style="color:#cbd5e1; font-size:14px; margin-bottom:15px;">প্রতিদিনের কাজগুলো সম্পূর্ণ করে এক্সট্রা কয়েন কালেক্ট করুন!</p>
                <div class="task-box">
        ...
        <div id="successModal" class="modal">
            <div class="modal-content">
                ...
                <h2 style="margin:20px 0 10px; color:white; font-size: 26px;">সম্পন্ন হয়েছে!</h2>
                <p style="color: #4ade80; font-size: 17px; font-weight: bold;">✅ ফাইলটি বটের ইনবক্সে পাঠানো হয়েছে।</p>
                <div class="notice-box" id="successNoticeBox">
                    <p><i class="fa-solid fa-triangle-exclamation" style="color: #fbbf24;"></i> <b>সতর্কতা:</b> কপিরাইট এড়াতে মুভিটি কিছুক্ষণ পর অটোমেটিক ডিলিট হয়ে যাবে। এখনই বট থেকে সেভ করে নিন!</p>
                </div>
        ...
        <script>
            ...
            async function fetchUserInfo() {
                try {
                    ...
                    if (isUserVip) {
                        ...
                        if (data.vip_expiry && !document.getElementById('vipExpiryText')) {
                            document.getElementById('dropdownMenu').insertAdjacentHTML('afterbegin',
                                `<div id="vipExpiryText" style="padding: 10px 15px; color: #4ade80; font-size: 13px; font-weight: bold; border-bottom: 1px solid #334155; text-align: center;"><i class="fa-regular fa-clock"></i> মেয়াদ: ${data.vip_expiry}</div>`
                            );
                        }
                    }
            ...
            function copyReferLink() {
                navigator.clipboard.writeText(document.getElementById('refLinkText').innerText).then(() => {
                    tg.showAlert("✅ রেফার লিংক কপি হয়েছে!");
                });
            }
            ...
            async function openLeaderboard() {
                ...
                try {
                    ...
                    const data = await res.json();
                    if (data.length === 0) return lbList.innerHTML = "<p style='color:gray; text-align:center;'>কোনো ডাটা পাওয়া যায়নি।</p>";
            ...
            async function loadReviews(title) {
                ...
                try {
                    ...
                    const data = await res.json();
                    if (data.length === 0) list.innerHTML = "<p style='color:gray; text-align:center; padding:10px;'>এখনো কেউ কমেন্ট করেনি।</p>";
                    else list.innerHTML = data.map(r => `<div class="review-item"><span>${'★'.repeat(r.rating)}${'☆'.repeat(5 - r.rating)}</span> <b style="color:#e2e8f0;">${r.name}</b>: <br><span style="color:#94a3b8;">${r.comment}</span></div>`).join('');
                } catch (e) {}
            }
            ...
            async function submitReview() {
                ...
                try {
                    ...
                    tg.showAlert("✅ ধন্যবাদ! আপনার রিভিউ যুক্ত হয়েছে।");
                } catch (e) {}
            }
            ...
            async function claimCheckin() {
                try {
                    ...
                    const data = await res.json();
                    if (data.ok) { tg.showAlert("🎉 অভিনন্দন! আপনি 10 Coins পেয়েছেন।"); fetchUserInfo(); }
                    else tg.showAlert(data.msg || "আপনি ইতিমধ্যে আজকের রিওয়ার্ড নিয়েছেন!");
                } catch (e) {}
            }
            ...
            async function convertCoins() {
                try {
                    ...
                    const data = await res.json();
                    if (data.ok) { tg.showAlert("✅ সফল! ৫০ কয়েন কেটে নেওয়া হয়েছে এবং ১ দিনের VIP চালু হয়েছে।"); fetchUserInfo(); }
                    else tg.showAlert(data.msg || "আপনার পর্যাপ্ত কয়েন নেই! (৫০ প্রয়োজন)");
                } catch (e) {}
            }
            ...
            async function submitPayment() {
                ...
                try {
                    ...
                    const data = await res.json();
                    if (data.ok) {
                        tg.showAlert("✅ পেমেন্ট রিকোয়েস্ট সফলভাবে পাঠানো হয়েছে! অ্যাডমিন যাচাই করার পর আপনার VIP চালু হবে।");
                        document.getElementById('vipModal').style.display = 'none';
                        document.getElementById('trxIdInput').value = '';
                    } else { tg.showAlert(data.msg || "TrxID আগে ব্যবহার করা হয়েছে অথবা ভুল!"); }
                } catch (e) {}
            }
            ...
            async function loadMovies(page = 1, signal = null) {
                ...
                try {
                    ...
                    if (data.movies && data.movies.length === 0) {
                        grid.innerHTML = `<p style='grid-column: span 2; text-align:center; color:#fbbf24; font-size: 18px; padding:40px;'>🚫 কোনো মুভি পাওয়া যায়নি!</p>`;
                    } else if (data.movies) {
            ...
            function openQualityModal(title) {
                ...
                let listHtml = movie.files.map(f => {
                    ...
                    return `<div class="rgb-border" onclick="handleQualityClick('${f.id}', ${f.is_unlocked})">
            ...
            function handleQualityClick(fileId, isUnlocked) {
                ...
                if (isUnlocked || isUserVip) {
                    ...
                    showDirectLinkModal("এই ভিডিও বা মুভিটি আনলক করতে নিচের বাটনে ক্লিক করুন। একটি নতুন পেইজ ওপেন হবে, সেখানে <b>১৫ সেকেন্ড</b> অপেক্ষা করুন।");
                }
            }
            ...
            function watchAdForCoins() {
                ...
                showDirectLinkModal("৫ কয়েন ফ্রী পেতে নিচের বাটনে ক্লিক করুন। একটি নতুন পেইজ ওপেন হবে, সেখানে <b>১৫ সেকেন্ড</b> অপেক্ষা করুন।");
            }
            ...
            async function claimAdReward() {
                try {
                    ...
                    const data = await res.json();
                    if (data.ok) { tg.showAlert("🎉 অভিনন্দন! আপনি লিংক ভিজিট করে ৫ কয়েন পেয়েছেন!"); fetchUserInfo(); loadTasks(); }
                } catch (e) {}
            }
            ...
            async function sendFile(id) {
                try {
                    ...
                    if (isUserVip) {
                        document.getElementById('successNoticeBox').innerHTML = `<p style="color:#4ade80;"><i class="fa-solid fa-crown" style="color: #fbbf24;"></i> <b>VIP সুবিধা:</b> এই ফাইলটি আপনার ইনবক্স থেকে কখনো অটো-ডিলিট হবে না।</p>`;
            ...
            async function loadRequests() {
                ...
                try {
                    ...
                    const data = await res.json();
                    if (data.length === 0) return reqList.innerHTML = "<p style='color:gray; text-align:center;'>কোনো রিকোয়েস্ট নেই।</p>";
            ...
            async function sendReq() {
                ...
                if (!text) return tg.showAlert('মুভির নাম লিখুন!');
                try {
                    ...
                    tg.showAlert('রিকোয়েস্ট বা ভোট সফলভাবে যোগ করা হয়েছে!');
                    loadRequests();
                } catch (e) {}
            }
            ...
            async function fetchChatMessages() {
                try {
                    ...
                    data.forEach(msg => {
                        ...
                        html += `<div class="chat-msg ${isMine ? 'mine' : 'others'}">
            ...
            function renderWheel() {
                ...
                for (let i = 0; i < 8; i++) {
                    ...
                    textDiv.style.cssText = `position:absolute; width:40px; height:50%; top:0; left:50%; margin-left:-20px; transform-origin:50% 100%; transform:rotate(${i * sliceAngle + (sliceAngle / 2)}deg); font-weight:900; font-size:18px; color:#000; text-align:center; padding-top:15px;`;
            ...
            function startSpin() {
                ...
                showDirectLinkModal("স্পিন আনলক করতে নিচের বাটনে ক্লিক করুন। একটি নতুন পেইজ ওপেন হবে, সেখানে <b>১৫ সেকেন্ড</b> অপেক্ষা করুন।");
            }
                ...
                setTimeout(async () => {
                    try {
                        ...
                        if (data.ok) {
                            spinsLeftToday = data.spins_left;
                            document.getElementById('spinsLeftText').innerText = spinsLeftToday;
                            if (reward > 0) { tg.showAlert(`🎉 অভিনন্দন! আপনি ${reward} কয়েন জিতেছেন!`); fetchUserInfo(); }
            ...
            async function loadTasks() {
                try {
                    ...
                    const data = await res.json();
                    let adCount = Math.min(data.ads || 0, 3);
                    let revCount = Math.min(data.reviews || 0, 2);
            ...
            // BUG FIX: claimTaskReward was referenced in HTML but never defined in original code
            async function claimTaskReward(taskType) {
                try {
                    const res = await fetch('/api/tasks/claim', {
                        method: 'POST', headers: {'Content-Type': 'application/json'},
                        body: JSON.stringify({ uid: uid, task_type: taskType, initData: INIT_DATA })
                    });
                    const data = await res.json();
                    if (data.ok) {
                        tg.showAlert("🎉 কয়েন সফলভাবে Claim করা হয়েছে!");
                        fetchUserInfo();
                        loadTasks();
                    } else {
                        tg.showAlert(data.msg || "Claim করা যায়নি!");
                    }
                } catch (e) {}
            }
    ...
    """
    html_code = (html_code
                 .replace("{{DIRECT_LINKS}}", dl_json)
                 .replace("{{ZONE_ID}}", zone_id)
                 .replace("{{TG_LINK}}", tg_url)
                 .replace("{{LINK_18}}", link_18)
                 .replace("{{AD_COUNT}}", str(required_ads))
                 .replace("{{BOT_USER}}", BOT_USERNAME)
                 .replace("{{BKASH_NO}}", bkash_no)
                 .replace("{{NAGAD_NO}}", nagad_no))
    return html_code
...
async def get_user_info(uid: int):
    user = await db.users.find_one({"user_id": uid})
    if not user:
        return {"vip": False, "is_admin": False, "refer_count": 0, "vip_expiry": None, "coins": 0, "badges": []}
    ...
    if unlocks >= 5:
        badges.append("🍿 Binge Watcher")
    if reviews_count >= 3:
        badges.append("✍️ Top Critic")
...
async def update_daily_task(uid: int, task_type: str):
    ...
    if task_type in tasks and isinstance(tasks[task_type], int):
        tasks[task_type] += 1
...
async def handle_checkin(data: CheckinModel):
    ...
    if not user:
        return {"ok": False, "msg": "ইউজার পাওয়া যায়নি।"}
    ...
    if data.action == "claim":
        ...
        if last_checkin.date() >= now.date():
            return {"ok": False, "msg": "আপনি ইতিমধ্যে আজকের রিওয়ার্ড নিয়ে নিয়েছেন!"}
        ...
        coins = user.get("coins", 0)
        if coins < 50:
            return {"ok": False, "msg": "আপনার কমপক্ষে ৫০ কয়েন প্রয়োজন!"}
        current_vip = user.get("vip_until", now)
        if current_vip < now:
            current_vip = now
        new_vip = current_vip + datetime.timedelta(days=1)
...
async def submit_payment(data: PaymentModel):
    ...
    if existing:
        return {"ok": False, "msg": "এই TrxID টি ইতিমধ্যে ব্যবহার করা হয়েছে!"}
    ...
    try:
        ...
        msg = (
            ...
            f"👤 ইউজার ID: <code>{data.uid}</code>\n"
            f"🏦 মেথড: {data.method.upper()}\n"
            f"🧾 TrxID: <code>{data.trx_id}</code>\n"
            f"💵 পরিমাণ: {data.price} BDT\n"
            f"⏳ প্যাকেজ: {pkg_name} VIP"
        )
...
async def upcoming_movies():
    movies = await db.upcoming.find().sort("added_at", -1).limit(10).to_list(10)
    ...
    try:
        ...
        if m:
            ...
            if is_vip:
                ...
                caption = (
                    f"🎥 <b>{title_text}</b>\n\n"
                    f"⏳ <b>সতর্কতা:</b> কপিরাইট এড়াতে মুভিটি <b>{del_minutes} মিনিট</b> পর অটো-ডিলিট হয়ে যাবে। "
                    "দয়া করে এখনই ফরওয়ার্ড বা সেভ করে নিন!\n\n"
                    "📥 Join: https://t.me/lifetimebackup2026"
                )
...
async def get_spin_status(uid: int):
    ...
    if spin_data.get("date") != now_date:
        return {"spins_left": 3}
    return {"spins_left": max(0, 3 - spin_data.get("count", 0))}
...
async def claim_task(data: TaskClaimModel):
    ...
    tasks = user.get("tasks", {})
    if tasks.get("date") != now_date:
        return {"ok": False, "msg": "মিশন সম্পূর্ণ হয়নি!"}
    ...
    return {"ok": False, "msg": "ইতিমধ্যে ক্লেইম করা হয়েছে বা মিশন সম্পূর্ণ হয়নি!"}
