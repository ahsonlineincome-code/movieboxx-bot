...
class ReplyMiddleware(BaseMiddleware):
    async def __call__(self, handler, event: types.Message, data: dict):
        if (event.chat.type == "private" and 
            event.from_user.id in admin_cache and 
            event.reply_to_message and 
            event.reply_to_message.message_id in reply_mapping):
            target_uid = reply_mapping[event.reply_to_message.message_id]
            try:
                await event.copy_to(chat_id=target_uid)
...
# ==========================================
# 4. Database Initialization & Caching
# ==========================================
...
async def update_monthly_users_bio():
    while True:
        try:
            ...
            await bot.set_my_short_description(f"{monthly_count:,} monthly users")
        except: pass
...
async def run_broadcast(admin_chat_id, photo_id, bcast_text, bcast_markup, del_minutes):
    ...
    try: await bot.send_message(admin_chat_id, f"✅ ব্রডকাস্ট শেষ! সফল: <b>{bcast_success}</b>", parse_mode="HTML")
    except: pass
...
async def start_cmd(message: types.Message, state: FSMContext):
    ...
    kb = [
        ...
        [types.InlineKeyboardButton(text="🚀 Join Channel", url=tg_link), types.InlineKeyboardButton(text="🔴 18+ Channel", url="https://t.me/+W5V9-mn08jMyYTE1")]
    ]
...
async def handle_user_messages(m: types.Message):
    ...
    if m.content_type not in allowed_types:
        return await m.answer("⚠️ এই ধরনের মেসেজ গ্রহণ করা হয় না।", parse_mode="HTML")
    try:
        ...
        if m.content_type == 'text':
            ...
            sent_msg = await m.copy_to(chat_id=OWNER_ID, caption=user_info + caption if caption else None, parse_mode="HTML", reply_markup=builder.as_markup())
...
async def reply_to_user_callback(c: types.CallbackQuery, state: FSMContext):
    ...
    await c.answer("✅ এই মেসেজে সরাসরি Reply করুন! আপলোড প্রসেস বাতিল হবে না।", show_alert=True)
...
async def toggle_protect(m: types.Message):
    ...
    await m.answer(f"✅ প্রোটেকশন {'অন 🔒' if new_status else 'অফ 🔓'}", parse_mode="HTML")
...
async def set_ad_count(m: types.Message):
    ...
    try:
        ...
        await m.answer(f"✅ অ্যাড সংখ্যা <b>{count}</b>", parse_mode="HTML")
...
async def set_tg_link(m: types.Message):
    ...
    try:
        ...
        await m.answer("✅ চ্যানেল লিংক আপডেট হয়েছে।", parse_mode="HTML")
    except: await m.answer("⚠️ /settg url", parse_mode="HTML")
...
async def del_movie_cmd(m: types.Message):
    ...
    try:
        ...
        await m.answer(f"✅ '{title}' ডিলিট হয়েছে!" if result.deleted_count > 0 else "⚠️ পাওয়া যায়নি", parse_mode="HTML")
...
async def add_vip_cmd(m: types.Message):
    ...
    try:
        ...
        await m.answer(f"✅ <code>{target_uid}</code> কে {days} দিনের VIP!", parse_mode="HTML")
...
async def receive_batch_year(m: types.Message, state: FSMContext):
    ...
    builder = InlineKeyboardBuilder()
    for i, cat in enumerate(CATEGORIES): builder.button(text=cat, callback_data=f"batselcat_{i}")
    builder.button(text="✅ Done", callback_data="batcats_done"); builder.adjust(2)
    await m.answer("✅ ক্যাটাগরি সিলেক্ট করুন।", reply_markup=builder.as_markup(), parse_mode="HTML")
...
async def process_batch_category_selection(c: types.CallbackQuery, state: FSMContext):
    i = int(c.data.split("_")[1]); cat = CATEGORIES[i]
    data = await state.get_data(); selected = data.get("categories", [])
    if cat in selected: selected.remove(cat)
    else: selected.append(cat)
    await state.update_data(categories=selected)
    builder = InlineKeyboardBuilder()
    for idx, ct in enumerate(CATEGORIES):
        builder.button(text=f"{'✅ ' if ct in selected else ''}{ct}", callback_data=f"batselcat_{idx}")
    builder.button(text="✅ Done", callback_data="batcats_done"); builder.adjust(2)
    try: await c.message.edit_text(f"✅ ক্যাটাগরি সিলেক্ট করুন ({len(selected)} টি):", reply_markup=builder.as_markup(), parse_mode="HTML")
    except: pass
    await c.answer()
...
async def receive_batch_quality(m: types.Message, state: FSMContext):
    data = await state.get_data(); files_list = data.get("files", [])
    ...
    await state.set_state(AdminStates.waiting_for_batch_file)
    await m.answer(f"✅ <b>{m.text.strip()}</b> যুক্ত হয়েছে!\nমোট: <b>{len(files_list)}</b>টি।\n\nপরবর্তী ফাইল পাঠান বা <b>/done</b> লিখুন।", parse_mode="HTML")

@dp.message(Command("done"), AdminStates.waiting_for_batch_file)
async def finish_batch_upload(m: types.Message, state: FSMContext):
    data = await state.get_data(); files_list = data.get("files", [])
    if not files_list: await state.clear(); return await m.answer("⚠️ কোনো ফাইল নেই!", parse_mode="HTML")
    await state.clear()
    ...
    tg_cfg = await db.settings.find_one({"id": "tg_link"})
    tg_link = tg_cfg.get("url", "https://t.me/addlist/MwbWNafSFK4yZjhl") if tg_cfg else "https://t.me/addlist/MwbWNafSFK4yZjhl"
    bcast_kb = [[types.InlineKeyboardButton(text="🎬 Watch Now", web_app=types.WebAppInfo(url=APP_URL))], [types.InlineKeyboardButton(text="🚀 Join Channel", url=tg_link), types.InlineKeyboardButton(text="🔴 18+", url="https://t.me/+W5V9-mn08jMyYTE1")]]
...
async def receive_movie_title(m: types.Message, state: FSMContext):
    ...
    await m.answer("✅ এবার <b>কোয়ালিটি</b> লিখুন।", parse_mode="HTML")
...
async def process_category_selection(c: types.CallbackQuery, state: FSMContext):
    ...
    builder = InlineKeyboardBuilder()
    for idx, ct in enumerate(CATEGORIES):
        builder.button(text=f"{'✅ ' if ct in selected else ''}{ct}", callback_data=f"selcat_{idx}")
    builder.button(text="✅ Done", callback_data="cats_done"); builder.adjust(2)
    try: await c.message.edit_text(f"✅ ক্যাটাগরি সিলেক্ট করুন ({len(selected)} টি):", reply_markup=builder.as_markup(), parse_mode="HTML")
    except: pass
    await c.answer()
...
async def finish_category_selection(c: types.CallbackQuery, state: FSMContext):
    ...
    if not selected: return await c.answer("⚠️ অন্তত ১টি সিলেক্ট করুন!", show_alert=True)
    await state.clear()
    await db.movies.insert_one({"title": data["title"], "quality": data["quality"], "photo_id": data["photo_id"], "file_id": data["file_id"], "file_type": data["file_type"], "year": data.get("year", "N/A"), "categories": selected, "clicks": 0, "created_at": datetime.datetime.utcnow()})
    await c.message.edit_text(f"🎉 <b>{data['title']} [{data['quality']}]</b> যুক্ত হয়েছে!", parse_mode="HTML")
    
    tg_cfg = await db.settings.find_one({"id": "tg_link"})
    tg_link = tg_cfg.get("url", "https://t.me/addlist/MwbWNafSFK4yZjhl") if tg_cfg else "https://t.me/addlist/MwbWNafSFK4yZjhl"
    bcast_kb = [[types.InlineKeyboardButton(text="🎬 Watch Now", web_app=types.WebAppInfo(url=APP_URL))], [types.InlineKeyboardButton(text="🚀 Join", url=tg_link), types.InlineKeyboardButton(text="🔴 18+", url="https://t.me/+W5V9-mn08jMyYTE1")]]
    time_cfg = await db.settings.find_one({"id": "del_time"}); del_minutes = time_cfg['minutes'] if time_cfg else 60
    asyncio.create_task(run_broadcast(c.from_user.id, data["photo_id"], f"🆕 <b>New Movie!</b>\n\n🎬 <b>{data['title']}</b>\n📺 {data['quality']}", types.InlineKeyboardMarkup(inline_keyboard=bcast_kb), del_minutes))
...
async def execute_broadcast(m: types.Message, state: FSMContext):
    if m.text and m.text.startswith("/"): await state.clear(); return await m.answer("⚠️ বাতিল হয়েছে।", parse_mode="HTML")
    await state.clear()
    prog_msg = await m.answer("⏳ <b>Broadcasting...</b>", parse_mode="HTML")
    total, success, blocked = await db.users.count_documents({}), 0, 0
    async for u in db.users.find():
        try: await m.copy_to(chat_id=u['user_id']); success += 1; await asyncio.sleep(0.05)
        except: blocked += 1
    try: await prog_msg.edit_text(f"✅ <b>Complete!</b>\n👥 Total: <b>{total}</b>\n✅ Success: <b>{success}</b>\n🚫 Blocked: <b>{blocked}</b>", parse_mode="HTML")
    except: pass
...
async def send_file(d: SendRequestModel):
    ...
    try:
        ...
        if m:
            ...
            is_protected = (await db.settings.find_one({"id": "protect_content"}) or {}).get("status", False)
            del_minutes = (await db.settings.find_one({"id": "del_time"}) or {}).get('minutes', 60)
            tg_link = (await db.settings.find_one({"id": "tg_link"}) or {}).get("url", "https://t.me/addlist/MwbWNafSFK4yZjhl")
            caption = f"🎥 <b>{m['title']} [{m.get('quality', '')}]</b>\n\n📥 Join: {tg_link}" + ("\n\n💎 VIP!" if is_vip else f"\n\n⏳ {del_minutes} মিনিট পর ডিলিট হবে!")
...
async def get_favs(uid: int):
    ...
    movies = await db.movies.aggregate([{"$match": {"title": {"$in": fav_titles}}}, {"$group": {"_id": "$title", "photo_id": {"$first": "$photo_id"}, "year": {"$first": "$year"}, "categories": {"$first": "$categories"}, "files": {"$push": {"id": {"$toString": "$_id"}, "quality": {"$ifNull": ["$quality", "Main"]}}}}}]).to_list(len(fav_titles))
