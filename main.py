...
async def toggle_protect(m: types.Message):
    ...
    await m.answer(f"✅ ফরোয়ার্ড প্রোটেকশন এখন <b>{'অন 🔒' if new_status else 'অফ 🔓'}</b>", parse_mode="HTML")
...
async def add_vip_cmd(m: types.Message):
    if m.from_user.id not in admin_cache: return
    try:
        args = m.text.split(); target_uid = int(args[1]); days = int(args[2]) if len(args) > 2 else 30
        now = datetime.datetime.utcnow(); user = await db.users.find_one({"user_id": target_uid})
...
async def execute_broadcast(m: types.Message, state: FSMContext):
    if m.text and m.text.startswith("/"):
        await state.clear(); await m.answer("⚠️ ব্রডকাস্ট বাতিল হয়েছে।", parse_mode="HTML"); return
    await state.clear()
...
async def reply_to_user_callback(c: types.CallbackQuery, state: FSMContext):
    ...
    await state.set_state(AdminStates.waiting_for_reply); await state.update_data(reply_user_id=user_id)
    await c.message.answer("✍️ আপনার মেসেজ লিখুন:"); await c.answer()

@dp.message(AdminStates.waiting_for_reply)
async def send_reply_to_user(m: types.Message, state: FSMContext):
    data = await state.get_data(); user_id = data.get("reply_user_id"); await state.clear()
    if user_id:
        try: await m.copy_to(chat_id=user_id); await m.answer("✅ রিপ্লাই পাঠানো হয়েছে!")
        except: await m.answer("❌ রিপ্লাই পাঠাতে ব্যর্থ হয়েছে।")
...
async def receive_movie_file(m: types.Message, state: FSMContext):
    ...
    await m.answer("✅ ফাইল পেয়েছি! এবার <b>পোস্টার (ছবি)</b> পাঠান।", parse_mode="HTML")
...
async def process_category_selection(c: types.CallbackQuery, state: FSMContext):
    index = int(c.data.split("_")[1]); cat = CATEGORIES[index]
    ...
    await c.message.edit_reply_markup(reply_markup=markup); await c.answer()
...
async def background_movie_broadcast(data, selected_cats):
    ...
    async for u in db.users.find():
        try:
            ...
            bcast_success += 1; await asyncio.sleep(0.1)
...
async def finish_category_selection(c: types.CallbackQuery, state: FSMContext):
    ...
    if LOG_CHANNEL_ID:
        try:
            ...
            await bot.send_photo(LOG_CHANNEL_ID, photo=data["photo_id"], caption=f"🎬 <b>New Movie</b>\n🏷 {data['title']}\n📺 {data['quality']}\n📅 {data.get('year', 'N/A')}", parse_mode="HTML", reply_markup=types.InlineKeyboardMarkup(inline_keyboard=log_kb))
        except: pass
...
async def handle_user_messages(m: types.Message):
    if m.content_type not in ['text']:
        await m.answer("⚠️ আমি শুধুমাত্র টেক্সট মেসেজ গ্রহণ করি।", parse_mode="HTML"); return
    try:
        builder = InlineKeyboardBuilder(); builder.button(text="✍️ রিপ্লাই", callback_data=f"reply_{m.from_user.id}")
...
async def admin_panel_ui(auth: bool = Depends(verify_admin)):
    return HTMLResponse('<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Admin</title><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css"><style>body{font-family:sans-serif;background:#0f172a;color:#cbd5e1;margin:0;padding:20px}.header{text-align:center;margin-bottom:30px}.header h1{font-size:28px;background:linear-gradient(45deg,#ff416c,#ff4b2b);-webkit-background-clip:text;-webkit-text-fill-color:transparent}.stats-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:20px;margin-bottom:40px}.stat-card{background:#1e293b;padding:20px;border-radius:16px;border:1px solid #334155}.stat-card h3{margin:0 0 10px;font-size:14px;color:#94a3b8;text-transform:uppercase}.stat-card .value{font-size:32px;font-weight:800;color:#fff}table{width:100%;border-collapse:collapse;min-width:600px}th{text-align:left;padding:15px;color:#94a3b8;font-size:12px;text-transform:uppercase;border-bottom:1px solid #334155}td{padding:15px;border-bottom:1px solid #334155;font-size:14px}tr:hover{background:rgba(255,255,255,0.03)}.delete-btn{background:rgba(239,68,68,0.2);color:#f87171;border:1px solid rgba(239,68,68,0.3);padding:6px 12px;border-radius:8px;cursor:pointer;font-weight:600}.delete-btn:hover{background:#ef4444;color:#fff}.view-badge{background:rgba(59,130,246,0.2);color:#60a5fa;padding:4px 10px;border-radius:12px;font-weight:600;font-size:12px}.table-container{background:#1e293b;border-radius:16px;border:1px solid #334155;overflow-x:auto}</style></head><body><div class="header"><h1>Admin Panel</h1></div><div class="stats-grid"><div class="stat-card"><h3>Total Users</h3><div class="value"><span id="totalUsers">0</span></div></div><div class="stat-card"><h3>Today Users</h3><div class="value"><span id="todayUsers">0</span></div></div><div class="stat-card"><h3>Total Clicks</h3><div class="value"><span id="totalClicks">0</span></div></div><div class="stat-card"><h3>Live (5m)</h3><div class="value"><span id="activeUsers">0</span></div></div></div><div class="table-container"><table><thead><tr><th>Title</th><th>Quality</th><th>Category</th><th>Views</th><th>Action</th></tr></thead><tbody id="movieTableBody"></tbody></table></div><script>async function fetchStats(){try{var r=await fetch("/api/admin/stats");var d=await r.json();document.getElementById("totalUsers").innerText=d.total_users;document.getElementById("todayUsers").innerText=d.today_users;document.getElementById("totalClicks").innerText=d.total_clicks;document.getElementById("activeUsers").innerText=d.active_users}catch(e){}}async function fetchMovies(){try{var r=await fetch("/api/admin/movies");var ms=await r.json();var tb=document.getElementById("movieTableBody");if(ms.length===0){tb.innerHTML="<tr><td colspan=5>No movies.</td></tr>";return}tb.innerHTML=ms.map(function(m){return "<tr id=row-"+m._id+"><td><strong>"+m.title+"</strong></td><td>"+(m.quality||"Main")+"</td><td>"+(m.categories||[]).join(", ")+"</td><td><span class=view-badge>"+(m.clicks||0)+"</span></td><td><button class=delete-btn onclick=deleteMovie(\'"+m._id+"\')>Delete</button></td></tr>"}).join("")}catch(e){}}async function deleteMovie(id){if(!confirm("Delete?"))return;try{var r=await fetch("/api/admin/movie/"+id,{method:"DELETE"});var d=await r.json();if(d.ok){document.getElementById("row-"+id).remove();fetchStats()}}catch(e){}}fetchStats();fetchMovies();setInterval(fetchStats,60000);</script></body></html>')
...
async def web_ui():
    ...
    # Build HTML safely - using string concatenation to avoid ALL escaping issues
    h = ""
    h += "<!DOCTYPE html><html lang='bn'><head><meta charset='UTF-8'>"
    h += "<meta name='viewport' content='width=device-width,initial-scale=1.0,maximum-scale=1.0,user-scalable=no'>"
    h += "<title>Movie Box</title>"
    h += "<script src='https://telegram.org/js/telegram-web-app.js'></script>"
    h += "<link rel='stylesheet' href='https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css'>"
    h += "<style>"
    h += "@import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700;800;900&display=swap');"
    h += "*{margin:0;padding:0;box-sizing:border-box}body{background:#0f172a;font-family:'Inter',sans-serif;color:#fff;overscroll-behavior-y:none;transition:background 0.3s}body.oled-mode{background:#000}"
    h += "#welcomeScreen{position:fixed;top:0;left:0;width:100%;height:100%;background:#0f172a;z-index:99999;display:flex;flex-direction:column;align-items:center;justify-content:center;transition:opacity 0.8s ease}#welcomeScreen.hide{opacity:0;visibility:hidden}"
    h += ".ws-brand{font-size:48px;font-weight:900;background:linear-gradient(45deg,#ff416c,#ff4b2b);-webkit-background-clip:text;-webkit-text-fill-color:transparent;animation:pulse 1.5s infinite}.ws-bn{font-size:18px;color:#94a3b8;margin-top:10px}"
    h += "@keyframes pulse{0%{transform:scale(1)}50%{transform:scale(1.05)}100%{transform:scale(1)}}"
    h += "header{display:flex;justify-content:center;align-items:center;padding:15px;border-bottom:1px solid #1e293b;position:sticky;top:0;background:rgba(15,23,42,0.95);backdrop-filter:blur(10px);z-index:1000;cursor:pointer}body.oled-mode header{background:rgba(0,0,0,0.95);border-color:#1a1a1a}"
    h += ".logo{display:flex;align-items:center;font-size:24px;font-weight:900;background:linear-gradient(45deg,#ff416c,#ff4b2b);-webkit-background-clip:text;-webkit-text-fill-color:transparent}"
    h += ".page-section{display:none;padding-bottom:80px}.page-section.active{display:block}"
    h += ".cat-row{display:flex;flex-wrap:wrap;gap:8px;padding:15px}.cat-chip{background:#1e293b;padding:8px 16px;border-radius:20px;white-space:nowrap;cursor:pointer;border:1px solid #ef4444;font-weight:600;font-size:12px;transition:0.3s;color:#cbd5e1}.cat-chip.active{background:linear-gradient(45deg,#ef4444,#dc2626);border-color:#ef4444;color:#fff;box-shadow:0 0 12px rgba(239,68,68,0.4)}"
    h += ".movie-list{padding:0 15px;display:flex;flex-direction:column;gap:15px}"
    h += ".movie-card{display:flex;background:rgba(30,41,59,0.6);border-radius:16px;overflow:hidden;border:1px solid #334155;cursor:pointer;transition:0.3s;position:relative}body.oled-mode .movie-card{background:#0a0a0a;border-color:#1a1a1a}.movie-card:active{transform:scale(0.98)}"
    h += ".movie-card img{width:110px;height:160px;object-fit:cover;flex-shrink:0}"
    h += ".movie-info{padding:12px;display:flex;flex-direction:column;justify-content:center;flex:1}.movie-title{font-size:16px;font-weight:700;margin-bottom:5px;line-height:1.3}.movie-meta{font-size:12px;color:#94a3b8;margin-bottom:8px;display:flex;gap:10px}.movie-cats{display:flex;flex-wrap:wrap;gap:5px}.movie-cat-tag{background:rgba(255,255,255,0.1);padding:3px 8px;border-radius:6px;font-size:10px;font-weight:600;color:#cbd5e1}"
    h += ".fav-btn{position:absolute;top:10px;right:10px;background:rgba(0,0,0,0.6);border:none;width:30px;height:30px;border-radius:50%;color:#fff;font-size:14px;cursor:pointer;display:flex;align-items:center;justify-content:center;z-index:10}.fav-btn.active{color:#ef4444}"
    h += ".adult-lock-overlay{position:absolute;top:0;left:0;width:110px;height:160px;background:rgba(0,0,0,0.7);display:flex;align-items:center;justify-content:center;color:#ef4444;font-size:30px;z-index:5}"
    h += ".floating-btn{position:fixed;right:15px;width:50px;height:50px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:20px;z-index:500;cursor:pointer;box-shadow:0 4px 15px rgba(0,0,0,0.5);border:2px solid #fff;text-decoration:none;color:#fff}.btn-tg{bottom:160px;background:linear-gradient(45deg,#24A1DE,#1b7ba8)}.btn-18{bottom:100px;background:linear-gradient(45deg,#ef4444,#b91c1c);font-weight:bold}"
    h += ".bottom-nav{position:fixed;bottom:0;left:0;width:100%;background:rgba(15,23,42,0.95);backdrop-filter:blur(10px);border-top:1px solid #1e293b;display:flex;justify-content:space-around;padding:10px 0;z-index:1000}body.oled-mode .bottom-nav{background:rgba(0,0,0,0.95);border-color:#1a1a1a}.nav-item{display:flex;flex-direction:column;align-items:center;color:#64748b;font-size:11px;font-weight:600;cursor:pointer;border:none;background:none}.nav-item i{font-size:20px;margin-bottom:3px}.nav-item.active{color:#ef4444}"
    h += ".modal{position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.85);display:none;align-items:flex-end;justify-content:center;z-index:3000}.modal-content{background:#1e293b;width:100%;max-width:400px;padding:25px;border-radius:20px 20px 0 0;max-height:90vh;overflow-y:auto;position:relative}body.oled-mode .modal-content{background:#000}"
    h += ".detail-img{width:100%;height:250px;object-fit:cover;border-radius:12px;margin-bottom:15px}.detail-title{font-size:22px;font-weight:800;margin-bottom:5px}.detail-meta{color:#94a3b8;font-size:14px;margin-bottom:15px}.close-icon{position:absolute;top:12px;right:15px;width:32px;height:32px;border-radius:50%;background:rgba(0,0,0,0.6);color:#fff;font-size:18px;display:flex;align-items:center;justify-content:center;cursor:pointer;border:none}"
    h += ".dl-file-btn{display:flex;align-items:center;justify-content:space-between;width:100%;padding:15px;background:#0f172a;border:1px solid #334155;color:#fff;font-weight:700;border-radius:10px;margin-bottom:10px;cursor:pointer}body.oled-mode .dl-file-btn{background:#050505;border-color:#1a1a1a}.dl-file-btn i{color:#ef4444;font-size:18px}.dl-file-btn.unlocked i{color:#10b981}"
    h += ".age-box{text-align:center}.age-btn{width:100%;padding:15px;border-radius:12px;font-weight:700;border:none;font-size:16px;cursor:pointer;margin-top:15px}.age-yes{background:#ef4444;color:#fff}.age-no{background:#334155;color:#fff}"
    h += ".ad-box{text-align:center;padding:20px}.ad-icon{font-size:60px;margin-bottom:10px;color:#fbbf24}.ad-title{color:#fbbf24;font-size:20px;font-weight:800;margin-bottom:15px}.ad-box-orange{background:#ea580c;color:#fff;padding:12px;border-radius:8px;margin-bottom:10px;font-weight:600}.ad-box-black{background:#000;color:#e2e8f0;padding:12px;border-radius:8px;margin-bottom:20px;font-size:14px}.ad-action-btn{width:100%;padding:15px;border-radius:8px;font-weight:700;border:none;font-size:16px;cursor:pointer;margin-bottom:10px}.btn-ad-open{background:#ea580c;color:#fff}.btn-ad-unlock{background:#10b981;color:#fff}.btn-ad-tryagain{background:#ef4444;color:#fff}"
    h += ".search-box{padding:0 15px 15px}.search-input{width:100%;padding:14px;border-radius:12px;border:none;outline:none;background:#1e293b;color:#fff;font-size:15px;border:1px solid #334155}body.oled-mode .search-input{background:#0a0a0a;border-color:#1a1a1a}"
    h += ".profile-card{background:#1e293b;margin:15px;border-radius:16px;padding:20px;border:1px solid #334155}body.oled-mode .profile-card{background:#0a0a0a;border-color:#1a1a1a}.profile-action-btn{display:block;width:100%;padding:14px;border-radius:12px;font-weight:700;text-align:center;margin-bottom:10px;border:none;color:#fff;text-decoration:none;cursor:pointer;font-size:15px;transition:0.2s}.profile-action-btn:active{transform:scale(0.97)}.btn-dark-mode{background:#334155;display:flex;align-items:center;justify-content:center;gap:10px}.btn-fb{background:#1877F2}.btn-main-ch{background:#24A1DE}.btn-18-ch{background:#ef4444}.btn-sax-grp{background:#8B5CF6}"
    h += ".skeleton{background:#1e293b;border-radius:12px;height:160px;position:relative;overflow:hidden}.skeleton::after{content:'';position:absolute;top:0;left:0;width:100%;height:100%;background:linear-gradient(90deg,transparent,rgba(255,255,255,0.05),transparent);animation:shimmer 1.5s infinite}@keyframes shimmer{0%{transform:translateX(-100%)}100%{transform:translateX(100%)}}"
    h += ".join-channel-btn{display:block;width:100%;padding:15px;border-radius:12px;background:#24A1DE;color:#fff;font-weight:700;text-decoration:none;font-size:16px;text-align:center;margin-top:15px;margin-bottom:10px}"
    h += "</style></head><body>"
    
    # Welcome Screen
    h += "<div id='welcomeScreen'><div class='ws-brand'>Movie Box</div><div class='ws-bn'>মুভি বক্স জগতে স্বাগতম</div></div>"
    
    # Header
    h += "<header onclick='switchTab(\"home\")'><div class='logo'>🎬 Movie Box</div></header>"
    
    # Home Tab
    h += "<div id='tabHome' class='page-section active'>"
    h += "<div class='search-box'><input type='text' id='searchInput' class='search-input' placeholder='🔍 খুঁজুন...'></div>"
    h += "<div class='cat-row'>"
    h += "<div class='cat-chip active' onclick='filterCat(\"Home\",this)'>HOME</div>"
    h += "<div class='cat-chip' onclick='filterCat(\"Bangla\",this)'>BANGLA</div>"
    h += "<div class='cat-chip' onclick='filterCat(\"Bangla Dubbed\",this)'>BANGLA DUBBED</div>"
    h += "<div class='cat-chip' onclick='filterCat(\"Hindi Dubbed\",this)'>HINDI DUBBED</div>"
    h += "<div class='cat-chip' onclick='filterCat(\"Hollywood\",this)'>HOLLYWOOD</div>"
    h += "<div class='cat-chip' onclick='filterCat(\"Web Series\",this)'>WEB SERIES</div>"
    h += "<div class='cat-chip' onclick='filterCat(\"K-Drama\",this)'>K-DRAMA</div>"
    h += "<div class='cat-chip' onclick='filterCat(\"Anime\",this)'>ANIME</div>"
    h += "<div class='cat-chip' onclick='filterCat(\"Horror\",this)'>HORROR</div>"
    h += "<div class='cat-chip' onclick='verify18(this)'>ADULT CONTENT</div>"
    h += "</div><div class='movie-list' id='movieListHome'><div class='skeleton'></div><div class='skeleton'></div></div></div>"
    
    # Search Tab
    h += "<div id='tabSearch' class='page-section'><div class='search-box' style='padding-top:15px'><input type='text' id='searchInputMain' class='search-input' placeholder='🔍 সার্চ...' oninput='searchMovies()'></div><div class='movie-list' id='movieListSearch'></div></div>"
    
    # Fav Tab
    h += "<div id='tabFav' class='page-section'><h3 style='padding:15px;color:#fbbf24'>❤️ ফেভারিট</h3><div class='movie-list' id='movieListFav'></div></div>"
    
    # Surprise Tab
    h += "<div id='tabSurprise' class='page-section'><div style='display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:60vh;text-align:center;padding:20px'><div style='font-size:80px;margin-bottom:20px;animation:pulse 1.5s infinite'>🎲</div><h2 style='margin-bottom:15px;color:#fbbf24'>মুভি রুলেট!</h2><p style='color:#94a3b8;margin-bottom:30px'>কী দেখবেন ঠিক করতে পারছেন না?</p><button onclick='loadSurprise()' style='padding:15px 40px;background:linear-gradient(45deg,#ff416c,#ff4b2b);color:#fff;border:none;border-radius:30px;font-size:18px;font-weight:800;cursor:pointer'>🎲 Surprise Me!</button></div></div>"
    
    # Profile Tab
    h += "<div id='tabProfile' class='page-section'><div class='profile-card'><div style='text-align:center;margin-bottom:20px'><h2 id='profileName'>User</h2></div>"
    h += "<button class='profile-action-btn btn-dark-mode' onclick='toggleOledMode()'>🌙 ডার্ক মোড (OLED) <span id='darkModeStatus'>OFF</span></button>"
    h += "<a href='https://facebook.com/' class='profile-action-btn btn-fb' target='_blank'>📘 Facebook Group</a>"
    h += "<a href='https://t.me/addlist/MwbWNafSFK4yZjhl' class='profile-action-btn btn-main-ch' target='_blank'>🚀 Main Channel</a>"
    h += "<a href='https://t.me/+W5V9-mn08jMyYTE1' class='profile-action-btn btn-18-ch' target='_blank'>🔴 18+ Channel</a>"
    h += "<a href='#' class='profile-action-btn btn-sax-grp' target='_blank'>🔥 Sax Group</a>"
    h += "</div></div>"
    
    # Floating Buttons
    h += "<a href='https://t.me/addlist/MwbWNafSFK4yZjhl' class='floating-btn btn-tg'><i class='fa-brands fa-telegram'></i></a>"
    h += "<a href='https://t.me/+W5V9-mn08jMyYTE1' class='floating-btn btn-18'>18+</a>"
    
    # Bottom Nav
    h += "<div class='bottom-nav'>"
    h += "<button class='nav-item active' onclick='switchTab(\"home\",this)'><i class='fa-solid fa-house'></i>Home</button>"
    h += "<button class='nav-item' onclick='switchTab(\"search\",this)'><i class='fa-solid fa-magnifying-glass'></i>Search</button>"
    h += "<button class='nav-item' onclick='switchTab(\"fav\",this)'><i class='fa-solid fa-heart'></i>Favorites</button>"
    h += "<button class='nav-item' onclick='switchTab(\"surprise\",this)'><i class='fa-solid fa-dice'></i>Surprise</button>"
    h += "<button class='nav-item' onclick='switchTab(\"profile\",this)'><i class='fa-solid fa-user'></i>Profile</button>"
    h += "</div>"
    
    # Age Modal
    h += "<div id='ageModal' class='modal'><div class='modal-content age-box'><h2 style='color:#ef4444'>⚠️ বয়স সীমাবদ্ধতা</h2><p style='color:#cbd5e1;margin:15px 0'>আপনার বয়স কি ১৮ বছরের বেশি?</p><button class='age-btn age-yes' onclick='access18()'>হ্যাঁ, আমি ১৮+</button><button class='age-btn age-no' onclick='closeModal(\"ageModal\")'>না</button></div></div>"
    
    # Detail Modal
    h += "<div id='detailModal' class='modal'><div class='modal-content'><button class='close-icon' onclick='closeModal(\"detailModal\")'><i class='fa-solid fa-xmark'></i></button><img id='detailImg' class='detail-img' src=''><h2 id='detailTitle' class='detail-title'></h2><div id='detailMeta' class='detail-meta'></div><div id='detailCats' style='margin-bottom:15px'></div><div id='fileButtonsContainer'></div></div></div>"
    
    # Ad Modal
    h += "<div id='adModal' class='modal'><div class='modal-content ad-box'><div class='ad-icon'>⚠️</div><h2 class='ad-title'>সতর্কতা!</h2><div class='ad-box-orange'>ডাউনলোড করতে হলে অবশ্যই বিজ্ঞাপন দেখুন!</div><div class='ad-box-black'>লিংকে ক্লিক করে বিজ্ঞাপনটি দেখুন এবং কমপক্ষে <b>১০ সেকেন্ড</b> পর ফিরে এসে নিচের বাটনে ক্লিক করুন।</div><button class='ad-action-btn btn-ad-open' id='adClickBtn' onclick='openAdLink()'>বিজ্ঞাপন খুলুন</button><button class='ad-action-btn btn-ad-unlock' id='adVerifyBtn' onclick='checkAdWatched()' style='display:none'>✅ অ্যাড দেখে ফিরে এসেছি</button><button class='ad-action-btn btn-ad-tryagain' id='adTryAgainBtn' onclick='resetAdModal()' style='display:none'>TRY AGAIN</button></div></div>"
    
    # Success Modal
    h += "<div id='successModal' class='modal'><div class='modal-content' style='text-align:center;padding-top:40px'><i class='fa-solid fa-circle-check' style='font-size:70px;color:#4ade80;margin-bottom:20px'></i><h2>ফাইল পাঠানো হয়েছে!</h2><p style='color:#94a3b8;margin-top:10px'>বট চেক করুন।</p><a href='https://t.me/addlist/MwbWNafSFK4yZjhl' target='_blank' class='join-channel-btn'>🚀 Join Channel</a><button class='dl-file-btn unlocked' onclick='closeModal(\"successModal\");tg.close()'><i class='fa-solid fa-check'></i> বটে যান</button></div></div>"
    
    # JavaScript
    h += "<script>"
    h += "var tg=window.Telegram.WebApp;tg.expand();"
    h += "var DIRECT_LINKS=" + dl_json + ";var ADULT_DIRECT_LINKS=" + adl_json + ";var INIT_DATA=tg.initData||'';"
    h += "var uid=tg.initDataUnsafe&&tg.initDataUnsafe.user?tg.initDataUnsafe.user.id:0;var isUserVip=false;var activeCat='Home';var userFavs=[];var active18Btn=null;var activeFileId=null;var activeIsAdult=false;var adStartTime=0;var currentViewMovies=[];"
    h += "setTimeout(function(){document.getElementById('welcomeScreen').classList.add('hide')},2500);"
    h += "if(tg.initDataUnsafe&&tg.initDataUnsafe.user){document.getElementById('profileName').innerText=tg.initDataUnsafe.user.first_name;}"
    h += "async function fetchUserInfo(){try{var r=await fetch('/api/user/'+uid);var d=await r.json();isUserVip=d.vip}catch(e){}}"
    h += "function switchTab(tabName,btnEl){document.querySelectorAll('.page-section').forEach(function(el){el.classList.remove('active')});document.querySelectorAll('.nav-item').forEach(function(el){el.classList.remove('active')});if(tabName==='home'){activeCat='Home';document.querySelectorAll('.cat-chip').forEach(function(el){el.classList.remove('active')});var fc=document.querySelector('.cat-chip');if(fc)fc.classList.add('active')}var tabId='tab'+tabName.charAt(0).toUpperCase()+tabName.slice(1);document.getElementById(tabId).classList.add('active');if(btnEl)btnEl.classList.add('active');if(tabName==='home')loadHomeMovies();if(tabName==='fav')loadFavorites();window.scrollTo({top:0,behavior:'smooth'})}"
    h += "function filterCat(cat,btnEl){activeCat=cat;document.querySelectorAll('.cat-chip').forEach(function(el){el.classList.remove('active')});btnEl.classList.add('active');loadHomeMovies()}"
    h += "function verify18(btnEl){active18Btn=btnEl;if(localStorage.getItem('isAdult')){if(btnEl)filterCat('Adult Content',btnEl)}else{document.getElementById('ageModal').style.display='flex'}}"
    h += "function access18(){localStorage.setItem('isAdult','true');closeModal('ageModal');if(active18Btn){filterCat('Adult Content',active18Btn)}else{loadHomeMovies()}}"
    h += "function closeModal(id){document.getElementById(id).style.display='none'}"
    h += "function toggleOledMode(){document.body.classList.toggle('oled-mode');var sEl=document.getElementById('darkModeStatus');if(document.body.classList.contains('oled-mode')){sEl.innerText='ON';localStorage.setItem('oledMode','true')}else{sEl.innerText='OFF';localStorage.setItem('oledMode','false')}}"
    h += "if(localStorage.getItem('oledMode')==='true'){document.body.classList.add('oled-mode');document.getElementById('darkModeStatus').innerText='ON'}"
    h += "async function loadHomeMovies(){var list=document.getElementById('movieListHome');list.innerHTML='<div class=\"skeleton\"></div>';try{var r=await fetch('/api/list?cat='+activeCat+'&uid='+uid);var d=await r.json();currentViewMovies=d.movies||[];if(currentViewMovies.length>0){list.innerHTML=currentViewMovies.map(function(m,i){return createMovieCard(m,i)}).join('')}else{list.innerHTML='<p style=\"text-align:center;color:#64748b;padding:30px\">কোনো মুভি পাওয়া যায়নি!</p>'}}catch(e){list.innerHTML='<p style=\"text-align:center;color:#ef4444;padding:30px\">Error!</p>'}}"
    h += "async function searchMovies(){var q=document.getElementById('searchInputMain').value.trim();var list=document.getElementById('movieListSearch');if(!q){list.innerHTML='';return}try{var r=await fetch('/api/list?q='+encodeURIComponent(q)+'&uid='+uid);var d=await r.json();currentViewMovies=d.movies||[];if(currentViewMovies.length>0){list.innerHTML=currentViewMovies.map(function(m,i){return createMovieCard(m,i)}).join('')}else{list.innerHTML='<p style=\"text-align:center;color:#64748b\">খুঁজে পাওয়া যায়নি!</p>'}}catch(e){}}"
    h += "function createMovieCard(m,index){var isFav=userFavs.indexOf(m._id)!==-1;var isAdult=m.categories&&m.categories.indexOf('Adult Content')!==-1;var isVerified=localStorage.getItem('isAdult')==='true';var catsHtml=(m.categories||[]).map(function(c){return '<span class=\"movie-cat-tag\">'+c+'</span>'}).join('');var imgSrc=(isAdult&&!isVerified)?'https://via.placeholder.com/110x160/1e293b/ef4444?text=18%2B':'/api/image/'+m.photo_id;var lockOverlay=(isAdult&&!isVerified)?'<div class=\"adult-lock-overlay\"><i class=\"fa-solid fa-lock\"></i></div>':'';var clickAction=(isAdult&&!isVerified)?'onclick=\"verify18(null)\"':'onclick=\"openDetail('+index+')\"';var safeId=m._id.replace(/'/g,\"\\\\'\");return '<div class=\"movie-card\" '+clickAction+'><div style=\"position:relative;flex-shrink:0\"><img src=\"'+imgSrc+'\" style=\"width:110px;height:160px;object-fit:cover\">'+lockOverlay+'</div><div class=\"movie-info\"><div class=\"movie-title\">'+m._id+'</div><div class=\"movie-meta\"><span>'+(m.year||'N/A')+'</span><span>'+(m.files?m.files.length:0)+' Files</span></div><div class=\"movie-cats\">'+catsHtml+'</div></div><button class=\"fav-btn '+(isFav?'active':'')+'\" onclick=\"event.stopPropagation();toggleFav(\\''+safeId+'\\',this)\"><i class=\"fa-solid fa-heart\"></i></button></div>'}"
    h += "function openDetail(index){var m=currentViewMovies[index];if(!m)return;document.getElementById('detailImg').src='/api/image/'+m.photo_id;document.getElementById('detailTitle').innerText=m._id;document.getElementById('detailMeta').innerHTML='<span>'+(m.year||'N/A')+'</span>';document.getElementById('detailCats').innerHTML=(m.categories||[]).map(function(c){return '<span class=\"movie-cat-tag\">'+c+'</span>'}).join(' ');var isAdult=m.is_adult||false;var btnsHtml=m.files.map(function(f){var isFree=f.is_unlocked||isUserVip;return '<button class=\"dl-file-btn '+(isFree?'unlocked':'')+'\" onclick=\"handleFileClick(\\''+f.id+'\\','+(isFree?'true':'false')+','+(isAdult?'true':'false')+')\"><span><i class=\"fa-solid fa-'+(isFree?'lock-open':'lock')+'\"></i> Download '+f.quality+'</span></button>'}).join('');document.getElementById('fileButtonsContainer').innerHTML=btnsHtml;document.getElementById('detailModal').style.display='flex'}"
    h += "function handleFileClick(fileId,isFree,isAdult){activeFileId=fileId;activeIsAdult=isAdult;if(isFree){sendFileRequest(fileId)}else{closeModal('detailModal');resetAdModal();document.getElementById('adModal').style.display='flex'}}"
    h += "function resetAdModal(){adStartTime=0;document.getElementById('adClickBtn').style.display='block';document.getElementById('adVerifyBtn').style.display='none';document.getElementById('adTryAgainBtn').style.display='none'}"
    h += "function openAdLink(){var linkToOpen=null;if(activeIsAdult&&ADULT_DIRECT_LINKS&&ADULT_DIRECT_LINKS.length>0){linkToOpen=ADULT_DIRECT_LINKS[Math.floor(Math.random()*ADULT_DIRECT_LINKS.length)]}else if(DIRECT_LINKS&&DIRECT_LINKS.length>0){linkToOpen=DIRECT_LINKS[Math.floor(Math.random()*DIRECT_LINKS.length)]}if(linkToOpen){tg.openLink(linkToOpen)}adStartTime=Date.now();document.getElementById('adClickBtn').style.display='none';document.getElementById('adVerifyBtn').style.display='block';document.getElementById('adTryAgainBtn').style.display='none'}"
    h += "function checkAdWatched(){if(adStartTime===0)return;var elapsed=Date.now()-adStartTime;if(elapsed>=10000){closeModal('adModal');sendFileRequest(activeFileId)}else{var remaining=Math.ceil((10000-elapsed)/1000);tg.showAlert('⚠️ আপনাকে আর '+remaining+' সেকেন্ড অপেক্ষা করতে হবে!');document.getElementById('adVerifyBtn').style.display='none';document.getElementById('adTryAgainBtn').style.display='block';document.getElementById('adTryAgainBtn').innerText='TRY AGAIN'}}"
    h += "async function sendFileRequest(fileId){try{var r=await fetch('/api/send',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({userId:uid,movieId:fileId,initData:INIT_DATA})});var d=await r.json();if(d.ok){closeModal('detailModal');document.getElementById('successModal').style.display='flex';fetchUserInfo()}else{tg.showAlert('⚠️ Failed!')}}catch(e){}}"
    h += "async function loadFavorites(){var list=document.getElementById('movieListFav');list.innerHTML='<div class=\"skeleton\"></div>';try{var r=await fetch('/api/favs/'+uid);var d=await r.json();userFavs=d.map(function(m){return m._id});currentViewMovies=d;if(d.length>0){list.innerHTML=d.map(function(m,i){return createMovieCard(m,i)}).join('')}else{list.innerHTML='<p style=\"text-align:center;color:#64748b;padding:30px\">কোনো ফেভারিট নেই!</p>'}}catch(e){}}"
    h += "async function toggleFav(title,btnEl){try{var r=await fetch('/api/fav/toggle',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({uid:uid,title:title,initData:INIT_DATA})});var d=await r.json();if(d.isFav){btnEl.classList.add('active');userFavs.push(title)}else{btnEl.classList.remove('active');userFavs=userFavs.filter(function(t){return t!==title})}}catch(e){}}"
    h += "async function loadSurprise(){try{var r=await fetch('/api/random');var d=await r.json();if(d.movie){currentViewMovies=[d.movie];openDetail(0)}else{tg.showAlert('⚠️ ডাটাবেসে কোনো মুভি নেই!')}}catch(e){}}"
    h += "document.getElementById('searchInput').addEventListener('focus',function(){document.querySelector('.nav-item:nth-child(2)').click();setTimeout(function(){document.getElementById('searchInputMain').focus()},100)});"
    h += "fetchUserInfo();loadHomeMovies();loadFavorites();"
    h += "</script></body></html>"
...
async def random_movie():
    movies = await db.movies.aggregate([{"$sample": {"size": 1}}]).to_list(1)
...
async def send_file(d: SendRequestModel):
    ...
    try:
        ...
        if m:
            ...
            protect_cfg = await db.settings.find_one({"id": "protect_content"}); is_protected = protect_cfg.get("status", False) if protect_cfg else False
            time_cfg = await db.settings.find_one({"id": "del_time"}); del_minutes = time_cfg['minutes'] if time_cfg else 60
            tg_cfg = await db.settings.find_one({"id": "tg_link"}); tg_link = tg_cfg.get("url", "https://t.me/addlist/MwbWNafSFK4yZjhl") if tg_cfg else "https://t.me/addlist/MwbWNafSFK4yZjhl"
            base_caption = f"🎥 <b>{m['title']} [{m.get('quality', '')}]</b>\n\n📥 Join: {tg_link}"
            caption = base_caption + ("\n\n💎 VIP: এই ফাইলটি কখনো ডিলিট হবে না!" if is_vip else f"\n\n⏳ সতর্কতা: এই ভিডিওটি {del_minutes} মিনিট পর অটো-ডিলিট হবে!")
