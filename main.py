...
async def receive_movie_year(m: types.Message, state: FSMContext):
    ...
    await m.answer("✅ এবার <b>ক্যাটাগরি সিলেক্ট</b> করুন।\n\n📌 একাধিক সিলেক্ট করতে পারবেন। শেষে ✅ Done চাপুন।", reply_markup=markup, parse_mode="HTML")
...
async def finish_category_selection(c: types.CallbackQuery, state: FSMContext):
    data = await state.get_data(); selected_cats = data.get("categories", [])
    if not selected_cats: return await c.answer("⚠️ অন্তত ১টি ক্যাটাগরি সিলেক্ট করুন!", show_alert=True)
    await state.clear()
    ...
    if LOG_CHANNEL_ID:
        try:
            ...
            await bot.send_photo(LOG_CHANNEL_ID, photo=data["photo_id"], caption=f"🎬 <b>New Movie</b>\n🏷 {data['title']}\n📺 {data['quality']}\n📅 {data.get('year', 'N/A')}\n📂 {', '.join(selected_cats)}", parse_mode="HTML", reply_markup=types.InlineKeyboardMarkup(inline_keyboard=log_kb))
        except: pass
    asyncio.create_task(background_movie_broadcast(data, selected_cats))
    await c.answer()

# এটি সবার শেষে - ভিডিও/ডকুমেন্ট রিসিভ করে মুভি আপলোড শুরু করে
@dp.message(F.video | F.document, StateFilter(None), lambda m: m.chat.type == "private" and m.from_user.id in admin_cache)
...
async def background_movie_broadcast(data, selected_cats):
    ...
    sent_ids = set()
    async for u in db.users.find():
        if u['user_id'] in sent_ids: continue
        sent_ids.add(u['user_id'])
        try:
...
async def handle_user_messages(m: types.Message):
    if m.content_type not in ['text']:
        await m.answer("⚠️ আমি শুধুমাত্র টেক্সট মেসেজ গ্রহণ করি।\n\n🎬 মুভি দেখতে 'Watch Now' বাটনে ক্লিক করুন।", parse_mode="HTML"); return
    try:
...
async def web_ui():
    ...
    <body>
        ...
        <div id="adModal" class="modal"><div class="modal-content ad-box"><div class="ad-icon">⚠️</div><h2 class="ad-title">সতর্কতা!</h2><div class="ad-box-orange">ডাউনলোড করতে হলে অবশ্যই বিজ্ঞাপন দেখুন!</div><div class="ad-box-black">লিংকে ক্লিক করে বিজ্ঞাপনটি দেখুন এবং কমপক্ষে <b>১০ সেকেন্ড</b> পর ফিরে এসে নিচের বাটনে ক্লিক করুন।</div><button class="ad-action-btn btn-ad-open" id="adClickBtn" onclick="openAdLink()">বিজ্ঞাপন খুলুন</button><button class="ad-action-btn btn-ad-unlock" id="adVerifyBtn" onclick="checkAdWatched()" style="display:none;">✅ অ্যাড দেখে ফিরে এসেছি</button><button class="ad-action-btn btn-ad-tryagain" id="adTryAgainBtn" onclick="resetAdModal()" style="display:none;">TRY AGAIN</button></div></div>
        ...
        <script>
            ...
            function createMovieCard(m, index) { let isFav = userFavs.includes(m._id); let isAdult = m.categories && m.categories.includes("Adult Content"); let isVerified = localStorage.getItem('isAdult') === 'true'; let catsHtml = (m.categories || []).map(function(c) { return '<span class="movie-cat-tag">' + c + '</span>'; }).join(''); let imgSrc = (isAdult && !isVerified) ? 'https://via.placeholder.com/110x160/1e293b/ef4444?text=18%2B' : '/api/image/' + m.photo_id; let lockOverlay = (isAdult && !isVerified) ? '<div class="adult-lock-overlay"><i class="fa-solid fa-lock"></i></div>' : ''; let clickAction = (isAdult && !isVerified) ? 'onclick="verify18(null)"' : 'onclick="openDetail(' + index + ')"'; return '<div class="movie-card" ' + clickAction + '><div style="position: relative; flex-shrink: 0;"><img src="' + imgSrc + '" style="width: 110px; height: 160px; object-fit: cover;">' + lockOverlay + '</div><div class="movie-info"><div class="movie-title">' + m._id + '</div><div class="movie-meta"><span>' + (m.year || 'N/A') + '</span><span>' + (m.files ? m.files.length : 0) + ' Files</span></div><div class="movie-cats">' + catsHtml + '</div></div><button class="fav-btn ' + (isFav ? 'active' : '') + '" onclick="event.stopPropagation(); toggleFav(\'' + m._id + '\', this)"><i class="fa-solid fa-heart"></i></button></div>'; }
            ...
            function checkAdWatched() { if (adStartTime === 0) return; let elapsed = Date.now() - adStartTime; if (elapsed >= 10000) { closeModal('adModal'); sendFileRequest(activeFileId); } else { let remaining = Math.ceil((10000 - elapsed) / 1000); tg.showAlert('⚠️ আপনাকে আর ' + remaining + ' সেকেন্ড অপেক্ষা করতে হবে!'); document.getElementById('adVerifyBtn').style.display = 'none'; document.getElementById('adTryAgainBtn').style.display = 'block'; } }
